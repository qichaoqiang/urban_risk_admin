<template>
  <div class="page">
        ...
        <Tabs value="name1">
          <TabPane label="建筑物信息" name="name1">
            ...
            <Form :disabled="isDisEditInfo" :model="baseInfo" ref="baseInfo1" :rules="baseInfoRules1" hide-required-mark
              ...
              <FormItem label="建筑物数量" prop="jzwsl">
                <InputNumber :min="0" v-model="baseInfo.jzwsl"></InputNumber>
              </FormItem>
              <FormItem label="建筑层数" prop="jzcs">
                <InputNumber :min="0" v-model="baseInfo.jzcs"></InputNumber>
              </FormItem>
              <FormItem label="建筑结构类型" prop="jzjglx">
                <Select clearable :transfer="true" v-model="baseInfo.jzjglx" placeholder="请选择" style="width: 120px">
                  <Option v-for="item in jzjglxList" :key="item" :value="item">{{item}}</Option>
                </Select>
              </FormItem>
              <FormItem label="是否设玻璃幕墙" prop="sfsblmq">
                <Select clearable :transfer="true" v-model="baseInfo.sfsblmq" placeholder="请选择" style="width: 78px">
                  <Option v-for="item in sfzgyyqList" :key="item.value" :value="item.value">{{item.name}}</Option>
                </Select>
              </FormItem>
              <FormItem label="是否住宿" prop="sfzs">
                <Select clearable :transfer="true" v-model="baseInfo.sfzs" placeholder="请选择" style="width: 78px">
                  <Option v-for="item in sfzgyyqList" :key="item.value" :value="item.value">{{item.name}}</Option>
                </Select>
              </FormItem>
              <FormItem label="住户数量" prop="zhsl" v-show="baseInfo.sfzs == 1">
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}人流信息`" v-model="showRltzModel"
      @on-visible-change="rltzModelChange">
      <div>
    ...
    methods: {
      async getBaseInfo() {
        ...
        let {
          ...
          data,
          message
        } = await api.getGcggjzBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          this.baseInfo = {
            jzzdmj: data.jzzdmj ? Number(data.jzzdmj) : 0,
            jzwsl: data.jzwsl ? Number(data.jzwsl) : 0,
            jzcs: data.jzcs ? Number(data.jzcs) : 0,
            sfsblmq: data.sfsblmq ? Number(data.sfsblmq) : 0,
            zhsl: data.zhsl ? Number(data.zhsl) : 0,
            jzjglx: data.jzjglx || "",
            sfzs: data.sfzs ? Number(data.sfzs) : 0,
          }
        }
        this.loading = false
      },
      ...
      async saveInfo1() {
        this.$refs.baseInfo1.validate(async valid => {
          if (valid) {
            let params = {
              gkdx_id: this.gkdx_id,
              jzzdmj: this.baseInfo.jzzdmj,
              jzwsl: this.baseInfo.jzwsl,
              jzcs: this.baseInfo.jzcs,
              sfsblmq: this.baseInfo.sfsblmq,
              sfzs: this.baseInfo.sfzs,
              jzjglx: this.baseInfo.jzjglx,
            }
            if (this.baseInfo.sfzs == 1) {
              params.zhsl = this.baseInfo.zhsl
            }
            let {
              status_code,
              message
            } = await api.addGcggjzBase(params);
            if (status_code == 200) {
              this.$Message.success('保存成功')
            }
          }
      ...
      async removeRltz(row) {
        let {
          status_code
        } = await api.deleteRltzInfo(row.id)
