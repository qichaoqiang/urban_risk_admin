...
<script>
	...
	export default {
		...
		computed: {
			baseInfoRules1() {
				return {
					jzzdmj: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
					jzwsl: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
					jzcs: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
					jzjglx: [{ required: true, message: '请选择', trigger: 'change' }],
					sfsblmq: [{ required: true, type: 'number', message: '请选择', trigger: 'change' }],
					sfzs: [{ required: true, type: 'number', message: '请选择', trigger: 'change' }],
					zhsl: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
				}
			},
		},
		methods: {
			async getBaseInfo() {
				...
				//   	this.loading = false
				// })
				let { status_code, data, message } = await api.getGcggjzBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					this.baseInfo = { 
			...
			async saveInfo1() {
				this.$refs.baseInfo1.validate(async valid => {
                    if (valid) {
						...
						let { status_code, message } = await api.addGcggjzBase(params);
						if(status_code == 200) {
							this.$Message.success('保存成功')
						}
                    }
