					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="物业负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
				        			<Input clearable v-model="baseInfo.wyfzr" placeholder="姓名"></Input>
				        		</Col>
				        		<Col span="16">
		...
		<Modal :title="`${modeType == 1 ? '新增' : '编辑'}危化品重大危险源`" v-model="showWhRiskModel" @on-visible-change="whRiskModelChange">
			<div>
				<Form :model="whRiskForm" label-position="left" :label-width="140">
			        ...
			        <FormItem label="投用时间">
			        	<DatePicker type="date" v-model="whRiskForm.kysj"  placeholder="请选择"></DatePicker>
			        </FormItem>
			        <FormItem label="当前状态">
		...
		<Modal :title="`${modeType == 1 ? '新增' : '编辑'}除尘设施`" v-model="showDeviceModel" @on-visible-change="deviceModelChange">
			<div>
				<Form :model="deviceForm" label-position="left" :label-width="140">
			        ...
			        <FormItem label="投用时间">
			            <DatePicker type="date" v-model="deviceForm.kysj"  placeholder="请选择"></DatePicker>
			        </FormItem>
			        <FormItem label="运行状态">
			            <Select clearable v-model="deviceForm.yyzt" placeholder="请选择">
			                <Option v-for="item in dqztList" :key="item" :value="item">{{item}}</Option>
			            </Select>
			        </FormItem>
				</Form>
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { xqmc, tyshxydm, hydm, jbr, jbrdh, jbryx, wyfzr, wyfzrdh, fgaqfzr, fgaqfzrdh, lngAndLat, dz, wydw, dyfw }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getGcggjzBase()
						if(status_code == 200) {
			...
			async saveWhRisk() {
				let params = {
					...this.whRiskForm,
					kysj: this.whRiskForm.kysj ? getDate(new Date(this.whRiskForm.kysj).getTime(), 'date') : '',
					gkdx_id: this.gkdx_id
				}
				if(this.modeType == 2) {
					params.id = this.id
			...
			async saveMainRisk() {
				let params = {
					...this.mainRiskForm,
					kysj: this.mainRiskForm.kysj ? getDate(new Date(this.mainRiskForm.kysj).getTime(), 'date') : '',
					gkdx_id: this.gkdx_id
				}
				if(this.modeType == 2) {
					params.id = this.id
			...
			async saveRim() {
				let params = {
					...this.rimForm,
					kysj: this.rimForm.kysj ? getDate(new Date(this.rimForm.kysj).getTime(), 'date') : '',
					gkdx_id: this.gkdx_id
				}
				if(this.modeType == 2) {
					params.id = this.id
