			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="工程规模">
				        	<Select clearable v-model="baseInfo.gcgm" placeholder="工程规模">
				                <Option v-for="item in gcgmList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="总库容（万立方）">
				        	<InputNumber :min="0" v-model="baseInfo.zkr"></InputNumber>
				        </FormItem>
				        <FormItem label="主坝最大坝高（m）">
				        	<InputNumber :min="0" v-model="baseInfo.zbzdbg"></InputNumber>
				        </FormItem>
				        <FormItem label="防洪保护人口">
				        	<Input clearable v-model="baseInfo.fhbhrk" placeholder="防洪保护人口"></Input>
				        </FormItem>
				        <FormItem label="地震区划等级（峰值加速度g）">
				            <InputNumber :min="0" v-model="baseInfo.dzqhdjfzjsdg"></InputNumber>
				        </FormItem>
				        <FormItem label="安全类别">
				        	<Select clearable v-model="baseInfo.aqlb" placeholder="安全类别">
				                <Option v-for="item in aqlbList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="竣工时间">
				        ...
				        <FormItem label="水库简介">
				        	<Input clearable type="textarea" :rows="4" v-model="baseInfo.skjj" placeholder="水库简介"></Input>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
	...
	export default {
		...
		data() {
			return {
				...
				yxztList: ['运行', '停运'],
				gcgmList: ['大（1）型', '大（2）型', '中型', '小（1）型', '小（2）型'],
				aqlbList: ['一类坝', '二类坝', '三类坝'],
				sfzgyyList: [
					{
						value: 0,
						name: '否'
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getSkdbBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { mc, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, jgsj, dz, gldw, tyshxydm, gcgm, fw, zkr, zbzdbg, dzqhdjfzjsdg, aqlb, skjj, fhbhrk } = this.form
					this.baseInfo = { mc, jbr, jbrdh, jbryx, fzr, fzrdh, dz, gldw, tyshxydm, gcgm, fw, aqlb, skjj, fhbhrk }
					this.baseInfo.zkr = zkr ? Number(zkr) : 0
					this.baseInfo.zbzdbg = zbzdbg ? Number(zbzdbg) : 0
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getSkdbBase()
						if(status_code == 200) {
			...
			openAreaModal() {	
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.fw) {
			            	let fw = JSON.parse(this.baseInfo.fw)
			            	let points = fw.map(item => {
			...
			openArea() {
				...
				this.polygonTool.addEventListener('draw', (e) => {
					// 获取绘制的多边形信息
					console.log(e);
					this.baseInfo.fw = JSON.stringify(e.currentLnglats)
				})
			},
			clearArea() {
				this.baseInfo.fw = ''
			...
			cancelArea() {
				...
				this.baseInfo.fw = this.fw || this.form.fw || '';
				this.showAreaModel = false
			},
			saveArea() {
				this.fw = this.baseInfo.fw;
				this.form.fw = this.baseInfo.fw;
				this.map.clearOverLays();
				this.map = null;
				this.polygonTool = null;
				this.showAreaModel = false
