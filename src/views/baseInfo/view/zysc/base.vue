		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="商品类型">
				        	<Input clearable v-model="baseInfo.splx" placeholder="服装、食品、家具、礼品、建材等"></Input>
				        </FormItem>
				        <FormItem label="运营状态">
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="市场负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
				        			<Input clearable v-model="baseInfo.scfzr" :data="areaList" placeholder="姓名"></Input>
				        		</Col>
				        		<Col span="16">
				        			<Input clearable v-model="baseInfo.scfzrdh" placeholder="电话"></Input>
				        		</Col>
				        	</Row>
				        </FormItem>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { scmc, tyshxydm, hydm, yysj1, yysj2, yyzt, gldw, splx, jbr, jbrdh, jbryx, scfzr, scfzrdh, fgaqfzr, fgaqfzrdh, dz, lngAndLat, dyfw, jzzdmj, ztjzcs, tysj, jzlx } = this.form
					this.baseInfo = { scmc, tyshxydm, hydm, yysj1, yysj2, yyzt, gldw, splx, jbr, jbrdh, jbryx, scfzr, scfzrdh, fgaqfzr, fgaqfzrdh, dz, lngAndLat, dyfw, jzlx }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getZyscBase()
						if(status_code == 200) {
