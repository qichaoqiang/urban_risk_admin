<template>
  <div class="page">
        ...
        <Tabs value="name1">
          <TabPane label="储存信息" name="name1">
            ...
            <Table :columns="cgColumns" :data="cgData">
              <template slot-scope="{ row }" slot="action" v-show="!isDisEditInfo">
                <Button type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editCgModel(row)">编辑</Button>
            ...
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="cgPage.pageSize" :total="cgPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeCgPage"
            ...
            <Table :columns="jylColumns" :data="jylData">
              ...
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editJylModel(row)">编辑</Button>
                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeJyl(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
            ...
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="jylPage.pageSize" :total="jylPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeJylPage"
                @on-page-size-change="handleChangeJylPageSize" />
            </Row>
          </TabPane>
          <TabPane label="重大危险源" name="name2">
            <Form :disabled="isDisEditInfo" :model="baseInfo" ref="baseInfo" :rules="baseInfoRules" hide-required-mark
              label-position="left" :label-width="140" style="width: 600px">
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}储罐信息`" v-model="showCgModel"
      @on-visible-change="cgModelChange">
      <div>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}加油量`" v-model="showJylModel"
      @on-visible-change="jylModelChange">
      <div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        cgColumns: [{
          ...
          title: '操作',
          fixed: 'right',
          width: this.isDisEditInfo ? 0 : 150,
          slot: 'action',
        }, ],
        cgData: [],
        cgForm: {
    ...
    methods: {
      async getBaseInfo() {
        ...
        Promise.all([api.getCgList(cgParams), api.getJylList(jylParams), api.getRimList(rimParams)]).then((
        result) => {
          let cgRes = result[0],
            jylRes = result[1],
            rimRes = result[2]
        ...
        let {
          ...
          data,
          message
        } = await api.getJyzBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          console.log(data)
          this.baseInfo = {
      ...
      async removeJyl(row) {
        let {
          status_code
        } = await api.deleteJylInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getJylList()
      },
      async saveJyl() {
