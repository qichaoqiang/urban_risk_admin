<template>
  <div class="page">
        ...
        <Tabs value="name1">
            ...
            <Table :columns="clxxctColumns" :data="clxxctData">
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editClxxctModel(row)">编辑</Button>
                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeClxxct(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
              </template>
            </Table>
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="clxxctPage.pageSize" :total="clxxctPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeClxxctPage"
          ...
          <TabPane label="线路信息" name="name2">
            ...
            <Table :columns="xlxxColumns" :data="xlxxData">
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editXlxxModel(row)">编辑</Button>
                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeXlxx(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
              </template>
            </Table>
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="xlxxPage.pageSize" :total="xlxxPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeXlxxPage"
          ...
          <TabPane label="运营状况" name="name3">
            ...
            <Table :columns="yyqkColumns" :data="yyqkData">
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editYyqkModel(row)">编辑</Button>
                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeYyqk(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
              </template>
            </Table>
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="yyqkPage.pageSize" :total="yyqkPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeYyqkPage"
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}车辆信息`" v-model="showClxxctModel"
      @on-visible-change="clxxctModelChange">
      <div>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}线路信息`" v-model="showXlxxModel"
      @on-visible-change="xlxxModelChange">
      <div>
        <Form :model="xlxxForm" ref="xlxx" :rules="xlxxRules" hide-required-mark label-position="left"
          :label-width="140">
          <FormItem label="线路名称">
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}运营状况`" v-model="showYyqkModel"
      @on-visible-change="yyqkModelChange">
      <div>
        <Form :model="yyqkForm" ref="yyqk" :rules="yyqkRules" hide-required-mark label-position="left"
          :label-width="140">
  ...
  export default {
    ...
    methods: {
      async getBaseInfo() {
        ...
        Promise.all([api.getClxxctList(clxxctParams), api.getXlxxList(xlxxParams), api.getYyqkList(xlxxParams)]).then(
          (result) => {
            let clxxctRes = result[0],
              xlxxRes = result[1],
              yyqkRes = result[2]
      ...
      async removeYyqk(row) {
        let {
          status_code
        } = await api.deleteYyqkInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getYyqkList()
      },
      async saveYyqk() {
    ...
    created() {
      ...
      if (this.isDisEditInfo) {
        this.clxxctColumns.pop()
        this.xlxxColumns.pop()
        this.yyqkColumns.pop()
      }
    },
    mounted() {
