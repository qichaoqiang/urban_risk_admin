    ...
    <div v-show="step == 1">
      <Row type="flex" justify="center">
        ...
        <Form disabled :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140"
          ...
          <FormItem label="投入使用时间">
          	<DatePicker type="date" v-model="baseInfo.trsysj"  placeholder="请选择"></DatePicker>
          </FormItem>
          <FormItem label="存放地址">
            <Input clearable v-model="baseInfo.cfdz" placeholder="存放地址"></Input>
          </FormItem>
          ...
          <FormItem label="功能用途">
            <Input clearable v-model="baseInfo.gnyt" placeholder="功能用途"></Input>
          </FormItem>
        </Form>
        </Col>
      </Row>
        ...
        <Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
          <FormItem label="队长">
            <Row type="flex" :gutter="20">
              ...
              <Col span="16">
              <Input clearable v-model="baseInfo.dh" placeholder="电话"></Input>
              </Col>
              <Col span="24">
              <Input clearable v-model="baseInfo.lxdw" placeholder="联系单位"></Input>
              </Col>
            </Row>
          </FormItem>
        </Form>
    ...
    methods: {
      async getBaseInfo() {
        let {
          ...
          data,
          message
        } = await api.getYjjyzbBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          let {
            zbmc, lb, trsysj, cfdz, lxdw, lxr, dh, gnyt
          } = this.form
          this.baseInfo = {
            zbmc, lb, cfdz, lxdw, lxr, dh, gnyt
          }
          this.baseInfo.trsysj = trsysj ? new Date(trsysj) : ''
      ...
      async submit() {
        let params = {
          ...this.baseInfo,
          trsysj: this.baseInfo.trsysj ? getDate(new Date(this.baseInfo.trsysj).getTime(), 'date') : '',
        ...
        let {
          status_code,
          message
        } = await api.addYjjyzbBase(params);
        if (status_code == 200) {
