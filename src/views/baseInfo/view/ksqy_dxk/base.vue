<template>
	<div class="page">
		...
		<Modal :title="`${modeType == 1 ? '新增' : '编辑'}涉爆粉尘工艺`" v-model="showSbfcgyModel" @on-visible-change="sbfcgyModelChange">
			<div>
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, zgrs, glrysl, ksjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, ksdz, kqfw, jyfw, ktfs, bpzy, swtj, dztj, kckz }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getKsqydxkBase()
						if(status_code == 200) {
