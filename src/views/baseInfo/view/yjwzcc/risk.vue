<template>
  <div class="page">
      ...
      <Row type="flex" justify="center" style="height: 100%">
        ...
        <Tabs value="name1">
          <TabPane label="储存物资信息" name="name1">
            <part-title text="储存物资信息" :btns="['add']" @add="openCcwzxxModel"></part-title>
            <Table :columns="ccwzxxColumns" :data="ccwzxxData">
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editCcwzxxModel(row)">编辑</Button>
                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeCcwzxx(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
              </template>
            </Table>
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="ccwzxxPage.pageSize" :total="ccwzxxPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeCcwzxxPage"
                @on-page-size-change="handleChangeCcwzxxPageSize" />
            </Row>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}储存物资信息`" v-model="showCcwzxxModel"
      @on-visible-change="ccwzxxModelChange">
      <div>
        <Form :model="ccwzxxForm" label-position="left" :label-width="200">
          <FormItem label="物资名称">
            <Input v-model="ccwzxxForm.wzmc"></Input>
          </FormItem>
          <FormItem label="数量">
            <InputNumber :min="0" v-model="ccwzxxForm.sl"></InputNumber>
          </FormItem>
          <FormItem label="状态">
            <Input v-model="ccwzxxForm.zt"></Input>
          </FormItem>
        </Form>
      </div>
      <div slot="footer">
        <!-- <Button type="text" size="large" @click="showCcwzxxModel = false">取消</Button> -->
        <Button type="primary" size="large" @click="saveCcwzxx">保存</Button>
      </div>
    </Modal>
    ...
    data() {
      return {
        ...
        ccwzxxColumns: [{
          ...
          align: "center",
          render: (h, params) => {
            return h('span', params.index + (this.ccwzxxPage.pageIndex - 1) * this.ccwzxxPage.pageSize + 1);
          },
        }, {
          title: '物资名称',
          key: 'wzmc',
    ...
    methods: {
      async getBaseInfo() {
        ...
        let ccwzxxParams = {
          ...
          per_page: this.ccwzxxPage.pageSize,
          page: this.ccwzxxPage.pageIndex,
        }
        Promise.all([api.getCcwzxxList(ccwzxxParams)]).then((result) => {
          let ccwzxxRes = result[0]
          if (ccwzxxRes.status_code == 200) {
            this.ccwzxxData = ccwzxxRes.data.data.filter(item => item.gkdx_id == this.gkdx_id)
            this.ccwzxxPage.totalRow = ccwzxxRes.data.total
          }
          this.loading = false
        }).catch((error) => {
          console.log(error)
      ...
      async getCcwzxxList_() {
        ...
        let {
          status_code,
          data
        } = await api.getCcwzxxList(params)
        if (status_code == 200) {
          this.ccwzxxList = data.data
        }
      },
      handleChangeCcwzxxPage(val) {
        this.ccwzxxPage.pageIndex = val
        this.getCcwzxxList()
      },
      handleChangeCcwzxxPageSize(val) {
        this.ccwzxxPage.pageSize = val
        this.getCcwzxxList()
      },
      async getCcwzxxList() {
        let params = {
        ...
        if (status_code == 200) {
          this.ccwzxxData = data.data.filter(item => item.gkdx_id == this.gkdx_id)
          this.ccwzxxPage.totalRow = data.total
        }
      },
      openCcwzxxModel() {
        this.modeType = 1;
        this.showCcwzxxModel = true
      },
      editCcwzxxModel(row) {
        this.ccwzxxForm = {
          wzmc: row.wzmc,
      ...
      async removeCcwzxx(row) {
        let {
          status_code
        } = await api.deleteCcwzxxInfo(row.id)
      ...
      async saveCcwzxx() {
        ...
        let {
          status_code,
          message
        } = await api.addCcwzxxInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
          this.showCcwzxxModel = false
          this.getCcwzxxList()
        }
      },
