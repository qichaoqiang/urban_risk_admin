    ...
    <div v-show="step == 1">
      <Row type="flex" justify="center">
        ...
        <Form disabled :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140"
          ...
          <FormItem label="仓库地址">
            <Input clearable v-model="baseInfo.ckdz" placeholder="仓库地址"></Input>
          </FormItem>
          <FormItem label="物资类别" prop="dwlsdw">
            <Input clearable v-model="baseInfo.wzlb" placeholder="物资类别"></Input>
          </FormItem>
        ...
        <Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
          <FormItem label="保管人">
            <Row type="flex" :gutter="20">
              <Col span="8">
              <Input clearable v-model="baseInfo.bgr" placeholder="姓名"></Input>
              </Col>
              <Col span="16">
              <Input clearable v-model="baseInfo.bgrdh" placeholder="电话"></Input>
              </Col>
            </Row>
          </FormItem>
        </Form>
    ...
    methods: {
      async getBaseInfo() {
        let {
          ...
          data,
          message
        } = await api.getYjwzccBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          let {
            mc, ckdz, wzlb, ssdw, bgr, bgrdh
          } = this.form
          this.baseInfo = {
            mc, ckdz, wzlb, ssdw, bgr, bgrdh
          }
      ...
      async submit() {
        ...
        let {
          status_code,
          message
        } = await api.addYjwzccBase(params);
        if (status_code == 200) {
