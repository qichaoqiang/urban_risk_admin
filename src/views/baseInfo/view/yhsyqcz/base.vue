				...
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="经营许可证编号">
				        	<Input clearable v-model="baseInfo.jyxkzbh" placeholder="经营许可证编号"></Input>
				        </FormItem>
				        <FormItem label="有效期">
				        ...
				        <FormItem label="场站类型">
				            <Select clearable v-model="baseInfo.czlx" placeholder="场站类型">
				                <Option v-for="item in czlxList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="供应站等级" v-show="baseInfo.czlx == '瓶装供应站'">
				            <Select clearable v-model="baseInfo.gyzdj" placeholder="供应站等级">
				                <Option v-for="item in gyzdjList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="职工人数">
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { jydw, tyshxydm, zcdz, zgrs, glrysl, hyml, hydm, jyxkzbh, yxq, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, zddz, lngAndLat, zdfw, zdmj, jzmj, jyfw, czlx, gyzdj } = this.form
					this.baseInfo = { jydw, tyshxydm, zcdz, hydm, jyxkzbh, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, zddz, zdfw, jyfw, czlx, gyzdj }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getYhsyqczBase()
						if(status_code == 200) {
			...
			openAreaModal() {	
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.zdfw) {
			            	let zdfw = JSON.parse(this.baseInfo.zdfw)
			            	let points = zdfw.map(item => {
			...
			openArea() {
				...
				this.polygonTool.addEventListener('draw', (e) => {
					// 获取绘制的多边形信息
					console.log(e);
					this.baseInfo.zdfw = JSON.stringify(e.currentLnglats)
				})
			},
			clearArea() {
				this.baseInfo.zdfw = ''
			...
			cancelArea() {
				...
				this.baseInfo.zdfw = this.zdfw || this.form.zdfw || '';
				this.showAreaModel = false
			},
			saveArea() {
				this.zdfw = this.baseInfo.zdfw;
				this.form.zdfw = this.baseInfo.zdfw;
				this.map.clearOverLays();
				this.map = null;
				this.polygonTool = null;
				this.showAreaModel = false
