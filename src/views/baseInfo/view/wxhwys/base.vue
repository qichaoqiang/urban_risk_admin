			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="危险货物运输许可证">
				        	<Input clearable v-model="baseInfo.wxhwysxkz" placeholder="危险货物运输许可证"></Input>
				        </FormItem>
				        <FormItem label="成立时间">
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, zcdz, zgrs, glrysl, hyml, hydm, wxhwysxkz, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, lngAndLat, jyfw, clsj, jydz } = this.form
					this.baseInfo = { qymc, tyshxydm, zcdz, hydm, wxhwysxkz, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, jyfw, jydz }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getWxhwysBase()
						if(status_code == 200) {
