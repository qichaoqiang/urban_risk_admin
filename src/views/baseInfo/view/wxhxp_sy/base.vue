...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				console.log(this.gkdx_id)
				let { status_code, data, message } = await api.getWxhxpSyBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, zgrs, glrysl, hyml, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, scdz, lngAndLat, qyfw } = this.form
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, scdz, qyfw }
			...
			async nextStep() {
				...
				let { status_code, message } = await api.addWxhxpSyBase(params);
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getWxhxpSyBase()
						if(status_code == 200) {
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.qyfw) {
			            	console.log(this.baseInfo.qyfw)
