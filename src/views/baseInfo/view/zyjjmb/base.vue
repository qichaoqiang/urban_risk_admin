		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" disabled hide-required-mark label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="名称">
				            {{baseInfo.zyjjmbmc}}
				        </FormItem>
				        <FormItem label="编号">
				        	<Input clearable v-model="baseInfo.bh" placeholder="编号"></Input>
				        </FormItem>
				        ...
				        <FormItem label="功能类别">
				        	<Select clearable v-model="baseInfo.mbgnlb" placeholder="功能类别">
				                <Option v-for="item in mbgnlbList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="行业类别">
				        	<Select clearable v-model="baseInfo.mbhylb" placeholder="行业类别">
				                <Option v-for="item in mbhylbList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="空间类别">
				        	<Select clearable v-model="baseInfo.mbkjlb" placeholder="空间类别">
				                <Option v-for="item in mbkjlbList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="管理单位">
				        	<Input clearable v-model="baseInfo.gsgldw" placeholder="管理单位"></Input>
				        </FormItem>
				        <!-- <FormItem label="管理单位联系电话">
				        	<Input clearable v-model="baseInfo.scdz" placeholder="管理单位联系电话"></Input>
				        </FormItem> -->
				        <FormItem label="资产总额规模(万元)">
				            <InputNumber :min="0" v-model="baseInfo.zczegm"></InputNumber>
				        </FormItem>
				        <FormItem label="防护保障措施">
				        	<Input type="textarea" clearable v-model="baseInfo.fhbzcs" placeholder="地址"></Input>
				        </FormItem>
				        <FormItem label="是否易发次生灾害">
				        	<Select clearable v-model="baseInfo.sfyfcszh" placeholder="是否易发次生灾害">
				                <Option v-for="item in sfzgyyList" :key="item.value" :value="item.value">{{item.name}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="是否隐真示假">
				        	<Select clearable v-model="baseInfo.sfyzsj" placeholder="是否隐真示假">
				                <Option v-for="item in sfzgyyList" :key="item.value" :value="item.value">{{item.name}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="是否配备人防专业队伍">
				        	<Select clearable v-model="baseInfo.sfpbrfzydw" placeholder="是否配备人防专业队伍">
				                <Option v-for="item in sfzgyyList" :key="item.value" :value="item.value">{{item.name}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="人防专业队伍人数">
				            <InputNumber :min="0" v-model="baseInfo.rfzydwrs"></InputNumber>
				        </FormItem>
				        <FormItem label="所属单位">
				        	<Input clearable v-model="baseInfo.ssdw" placeholder="所属单位"></Input>
				        </FormItem>
				        <FormItem label="所属单位负责人">
				        	<Input clearable v-model="baseInfo.ssdwfzr" placeholder="所属单位负责人"></Input>
				        </FormItem>
				        <FormItem label="联系方式">
				        	<Input clearable v-model="baseInfo.lxfs" placeholder="联系方式"></Input>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
			...
			<!-- <Row type="flex" justify="center">
				<Col>
					<part-title text="图像信息" :btns="['add']" @add="openTxxxModel"></part-title>
					<Table :columns="txxxColumns" :data="baseInfo.txxx" style="width: 600px">
						<template slot-scope="{ row }" slot="path">
							<img :src="row.path" />
						</template>
						<template slot-scope="{ row, index }" slot="action">
				            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="ediTxxxModel(row, index)">编辑</Button>
				            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removTxxx(index)">
	...
	export default {
		...
		data() {
			return {
				...
				mbgnlbList: ['信息通信类', '能源动力类', '交通枢纽类', '生命线工程(公共基础设施类)', '次生灾害源', '重要物资储备仓库类', '重要生产基地类（军工基地潜力）', '重要科研基地类', '金融管理类', '其他'],
				mbhylbList: ['重要工矿企业', '科研基地', '桥梁', '铁道', '交通枢纽', '通信枢纽', '战备物资仓库', '储罐', '油库', '发电厂', '配电厂', '水库和供水(自来水厂)', '供热', '供气设施', '其他'],
				mbkjlbList: ['地面目标', '地下目标', '空中目标', '综合'],
				sfzgyyList: [
					{
						value: 0,
						name: '否'
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getZyjjmbBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { zyjjmbmc, bh, mbjb, mbgnlb, mbhylb, mbkjlb, dz, zczegm, ssdw, ssdwfzr, lxfs, fhbzcs, sfyzsj, sfyfcszh, sfpbrfzydw } = this.form
					this.baseInfo = { zyjjmbmc, bh, mbjb, mbgnlb, mbhylb, mbkjlb, dz, ssdw, ssdwfzr, lxfs, fhbzcs }
					this.baseInfo.sfyzsj = sfyzsj ? Number(sfyzsj) : 0
					this.baseInfo.zczegm = zczegm ? Number(zczegm) : 0
					this.baseInfo.sfpbrfzydw = sfpbrfzydw ? Number(sfpbrfzydw) : 0
					this.baseInfo.sfyfcszh = sfyfcszh ? Number(sfyfcszh) : 0
			...
			async submit() {
				...
				let { status_code, message } = await api.addZyjjmbBase(params);
				if(status_code == 200) {
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.lngAndLat) {
			            	this.map.centerAndZoom(new T.LngLat(this.baseInfo.lngAndLat.split(' ')[0] || e.point.lng, this.baseInfo.lngAndLat.split(' ')[1] || e.point.lat), 10);
			            }else {
