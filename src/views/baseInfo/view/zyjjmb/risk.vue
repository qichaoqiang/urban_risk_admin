				        ...
				        <TabPane label="视频信号" name="name1">
							...
							<Row type="flex" justify="end">
								<Page
				                    ...
				                    :page-size="spxhPage.pageSize"
				                    :total="spxhPage.totalRow"
				                    show-elevator
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeSpxhPage"
				                    @on-page-size-change="handleChangeSpxhPageSize"
				                />
							</Row>
				        </TabPane>
				    </Tabs>
	...
	export default {
		...
		methods: {
			...
			async getSpxhList() {
				...
				let { status_code, data } = await api.getSpxhList(params)
				if(status_code == 200) {
			...
			async removeSpxh(row) {
				let { status_code } = await api.deleteSpxhInfo(row.id)
				status_code == 200 && this.$Message.success('删除成功')
				this.getSpxhList()
			},
			async saveSpxh() {
				this.$refs.spxh.validate(async valid => {
                    if (valid) {
						...
						let { status_code, message } = await api.addSpxhInfo(params);
						if(status_code == 200) {
							this.$Message.success(message)
