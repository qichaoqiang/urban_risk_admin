<template>
	<div class="page">
			...
			<Row type="flex" justify="center">
				<Col span="22">
					...
					<Tabs value="name1">
				        ...
				        <TabPane label="生产装置/储存设施" name="name3">
							...
							<Table :columns="sczzColumns" :data="sczzData">
								...
								<template slot-scope="{ row }" slot="action">
						            <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px" @click="editSczzModel(row)">编辑</Button>
						            <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeSczz(row)">
								        <Button type="error" size="small" ghost>删除</Button>
								    </Poptip>
						        </template>
							</Table>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}危险化学品`" v-model="showWhModel" @on-visible-change="whModelChange">
			<div>
				<Form :model="whForm" label-position="left" :label-width="140">
			        ...
			        <FormItem label="属性分类">
			            <Select clearable v-model="whForm.sxfl" placeholder="请选择">
			                <Option v-for="item in sxflList" :key="item" :value="item">{{item}}</Option>
			            </Select>
			        </FormItem>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}危险工艺`" v-model="showMainRiskModel" @on-visible-change="mainRiskModelChange">
			<div>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}生产装置`" v-model="showSczzModel" @on-visible-change="sczzModelChange">
			<div>
				<Form :model="sczzForm" label-position="left" :label-width="140">
					<FormItem label="单元名称">
			        ...
			        <FormItem label="经纬度">
			        	<lng id="lng_box_sc" :lngAndLat.sync="sczzForm.lngAndLat"></lng>
			        ...
			        <FormItem label="区域范围">
			        	<qyfw id="area_box_sc" :qyfw.sync="sczzForm.dtfw"></qyfw>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}储存设施`" v-model="showCcssModel" @on-visible-change="ccssModelChange">
			<div>
				<Form :model="ccssForm" label-position="left" :label-width="140">
					<FormItem label="名称">
				...
				<div v-show="ccssForm.lx == '地上罐区' || ccssForm.lx == '埋地罐区'">
					...
					<Table :columns="gqscColumns" :data="gqscData">
						...
						<template slot-scope="{ row, index }" slot="action">
				            ...
				            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeGqsc(row, index)">
						        <Button type="error" size="small" ghost>删除</Button>
						    </Poptip>
				        </template>
					</Table>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}涉及危险化学品`" v-model="showSjwxhxpModel" @on-visible-change="sjwxhxpModelChange">
			<div>
				<Form :model="sjwxhxpForm" label-position="left" :label-width="140">
					<FormItem label="化学品名">
		...
		<Modal width="820" :title="`${modeType2 == 1 ? '新增' : '编辑'}罐区`" v-model="showGqscModel" @on-visible-change="gqscModelChange">
			<div>
				<Form :model="gqscForm" label-position="left" :label-width="140">
					<FormItem label="罐区名称">
