			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="120" style="width: 600px">
				        ...
				        <FormItem label="审批情况">
				        	<Select clearable v-model="baseInfo.spqk" placeholder="审批情况">
				                <Option v-for="item in spqkList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="投用时间">
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { csmc, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, tysj, zysj, dz, gldw, tyshxydm, spqk, cdfw } = this.form
					this.baseInfo = { csmc, jbr, jbrdh, jbryx, fzr, fzrdh, dz, gldw, tyshxydm, spqk, cdfw }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getZtxncBase()
						if(status_code == 200) {
