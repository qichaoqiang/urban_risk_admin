			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="酒店星级">
				        	<InputNumber :min="0" v-model="baseInfo.jdxj"></InputNumber>
				        </FormItem>
				        <FormItem label="开业时间">
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { dwmc, tyshxydm, hydm, yyzt, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, zcdz, kysj, jddz, jdxj } = this.form
					this.baseInfo = { dwmc, tyshxydm, hydm, yyzt, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, zcdz, jddz }
					this.baseInfo.kysj = kysj ? new Date(kysj) : ''
					this.baseInfo.jdxj = jdxj ? Number(jdxj) : 0
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getXjjdBase()
						if(status_code == 200) {
