				...
				<Col span="22">
					...
					<Tabs value="name1">
				        <TabPane label="建筑物信息" name="name1">
				        	...
				        	<Form :disabled="isDisEditInfo" :model="baseInfo" label-position="left" :label-width="120">
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				let { status_code, data, message } = await api.getXjjdBase(this.gkdx_id);
				if(status_code == 0) {
					console.log(data)
			...
			async saveInfo1() {
				...
				let { status_code, message } = await api.addXjjdBase(params);
				if(status_code == 200) {
					this.$Message.success('保存成功')
				}
			},
