		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="业态">
				        	<Select clearable v-model="baseInfo.yt" placeholder="业态">
				                <Option v-for="item in ytList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="规模">
				        	<Select clearable v-model="baseInfo.gm" placeholder="规模">
				                <Option v-for="item in gmList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="运营状态">
				        	<Select clearable v-model="baseInfo.yxzt" placeholder="运营状态">
				...
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        <FormItem label="建筑占地面积（㎡）">
				        	<InputNumber clearable v-model="baseInfo.jzzdmj"></InputNumber>
				        </FormItem>
				        <FormItem label="建筑类型">
				        ...
				        <FormItem label="主体建筑层数">
				        	<InputNumber clearable v-model="baseInfo.ztjzcs"></InputNumber>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="综合体负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
				        			<Input clearable v-model="baseInfo.zhtfzr" :data="areaList" placeholder="姓名"></Input>
				        		</Col>
				        		<Col span="16">
				        			<Input clearable v-model="baseInfo.zhtfzrdh" placeholder="电话"></Input>
				        		</Col>
				        	</Row>
				        </FormItem>
	...
	export default {
		...
		data() {
			return {
				...
				ytList: ['歌舞娱乐放映场所', '儿童游乐厅', '购物', '餐饮', '住宿', '展览', '会议厅', '多功能厅', '电影院', '超市', '其他'],
				gmList: ['小型', '中小型', '中型', '大型', '超大型（CBD级）'],
				yxztList: ['在运行', '封闭升级', '休业'],
				jzlxList: ['单体建筑', '组群建筑'],
				sfzgyyList: [
					{
						value: 0,
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getSmzhtBase()
						if(status_code == 200) {
