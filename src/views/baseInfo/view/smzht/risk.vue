<template>
	<div class="page">
					...
					<Tabs value="name1">
				        <TabPane label="人流信息" name="name1">
				        	...
				        	<Form :disabled="isDisEditInfo" :model="baseInfo" label-position="left" inline>
						        ...
						        <FormItem label="餐饮企业数量" :label-width="104" style="margin-right: 24px">
						        	<InputNumber :min="0" v-model="baseInfo.cyqysl"></InputNumber>
						        </FormItem>
							...
							<Row type="flex" justify="end">
								<Page
				                    ...
				                    :page-size="smzhtRlxxPage.pageSize"
				                    :total="smzhtRlxxPage.totalRow"
				                    show-elevator
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeSmzhtRlxxPage"
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}人流信息`" v-model="showSmzhtRlxxModel" @on-visible-change="smzhtRlxxModelChange">
			<div>
				<Form :model="smzhtRlxxForm" label-position="left" :label-width="200">
			        <FormItem label="近一周日均人流量">
			        	<InputNumber :min="0" v-model="smzhtRlxxForm.jyzrjrll"></InputNumber>
			        </FormItem>
			        <FormItem label="填报时间">
			            <DatePicker type="date" v-model="smzhtRlxxForm.tbsj"  placeholder="请选择"></DatePicker>
			        </FormItem>
				</Form>
			</div>
			<div slot="footer">
	            <!-- <Button type="text" size="large" @click="showSmzhtRlxxModel = false">取消</Button> -->
		        <Button type="primary" size="large" @click="saveSmzhtRlxx">保存</Button>
	        </div>
		</Modal>
				...
				smzhtRlxxColumns: [
					{
				        ...
				        align: "center",
				        render: (h, params) => {
				            return h('span',params.index + (this.smzhtRlxxPage.pageIndex- 1) * this.smzhtRlxxPage.pageSize + 1);
				        },
                    }, {
                        title: '近一周日均人流量',
                        key: 'jyzrjrll',
		...
		methods: {
			async getBaseInfo() {
				...
				let smzhtRlxxParams = {
					...
					per_page: this.smzhtRlxxPage.pageSize,
					page: this.smzhtRlxxPage.pageIndex,
				}
				let gdtrqParams = {
					gkdx_id: this.gkdx_id,
				...
				Promise.all([api.getSmzhtRlxxList(smzhtRlxxParams), api.getGdtrqList(gdtrqParams), api.getYhsyqList(yhsyqParams), api.getCjrlList(cjrlParams), api.getElseList(cjrlParams)]).then((result) => {
				  	let smzhtRlxxRes = result[0], gdtrqRes = result[1], yhsyqRes = result[2], cjrlRes = result[3], elseRes = result[4]
				  	if(smzhtRlxxRes.status_code == 200) {
				  		this.smzhtRlxxData = smzhtRlxxRes.data.data.filter(item => item.gkdx_id == this.gkdx_id)
				  		this.smzhtRlxxPage.totalRow = smzhtRlxxRes.data.total
				  	}
				...
				let { status_code, data, message } = await api.getSmzhtBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					this.baseInfo = { dpsl: data.dpsl ? Number(data.dpsl) : 0, dxdpsl: data.dxdpsl ? Number(data.dxdpsl) : 0, cyqysl: data.cyqysl ? Number(data.cyqysl) : 0, sjzdrnrs: data.sjzdrnrs ? Number(data.sjzdrnrs) : 0 }
				}
			},
			async nextStep() {
				let params = {
			...
			async saveInfo() {
				let params = {
					gkdx_id: this.gkdx_id,
					dpsl: this.baseInfo.dpsl,
					dxdpsl: this.baseInfo.dxdpsl,
					cyqysl: this.baseInfo.cyqysl,
					sjzdrnrs: this.baseInfo.sjzdrnrs
				}
				let { status_code, message } = await api.addSmzhtBase(params);
				if(status_code == 200) {
					this.$Message.success('保存成功')
				}
			},
			...
			async getSmzhtRlxxList_() {
				let params = {
					gkdx_id: this.gkdx_id,
					per_page: 1000,
					page: this.smzhtRlxxPage.pageIndex,
				}
				let { status_code, data } = await api.getSmzhtRlxxList(params)
				if(status_code == 200) {
					this.smzhtRlxxList = data.data
				}
			},
			handleChangeSmzhtRlxxPage(val) {
				this.smzhtRlxxPage.pageIndex = val
				this.getSmzhtRlxxList()
			},
			handleChangeSmzhtRlxxPageSize(val) {
				this.smzhtRlxxPage.pageSize = val
				this.getSmzhtRlxxList()
			},
			async getSmzhtRlxxList() {
				let params = {
					gkdx_id: this.gkdx_id,
					per_page: this.smzhtRlxxPage.pageSize,
					page: this.smzhtRlxxPage.pageIndex,
				...
				let { status_code, data } = await api.getSmzhtRlxxList(params)
				if(status_code == 200) {
					this.smzhtRlxxData = data.data.filter(item => item.gkdx_id == this.gkdx_id)
					this.smzhtRlxxPage.totalRow = data.total
				}
			},
			openSmzhtRlxxModel() {
				this.modeType = 1;
				this.showSmzhtRlxxModel = true
			},
			editSmzhtRlxxModel(row) {
				this.smzhtRlxxForm = {
			...
			async removeSmzhtRlxx(row) {
				let { status_code } = await api.deleteSmzhtRlxxInfo(row.id)
			...
			async saveSmzhtRlxx() {
				let params = {
					...this.smzhtRlxxForm,
					tbsj: this.smzhtRlxxForm.tbsj ? getDate(new Date(this.smzhtRlxxForm.tbsj).getTime(), 'year') : '',
					gkdx_id: this.gkdx_id
				}
				if(this.modeType == 2) {
					params.id = this.id
				}
				let { status_code, message } = await api.addSmzhtRlxxInfo(params);
				if(status_code == 200) {
					this.$Message.success(message)
					this.showSmzhtRlxxModel = false
					this.getSmzhtRlxxList()
				}
			},
