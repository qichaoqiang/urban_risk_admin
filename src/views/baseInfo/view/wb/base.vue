			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="营业面积（㎡）">
				        	<InputNumber clearable v-model="baseInfo.yymj"></InputNumber>
				        </FormItem>
				        <FormItem label="营业状态">
				        ...
				        <FormItem label="发证日期">
				            <DatePicker type="date" v-model="baseInfo.fzrq"  placeholder="请选择"></DatePicker>
				        </FormItem>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { mc, tyshxydm, hydm, yysj1, yysj2, yyzt, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, zcdz, kysj, fzrq, jydz, yymj } = this.form
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getWbBase()
						if(status_code == 200) {
