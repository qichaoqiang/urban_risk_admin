			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" disabled hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="公共建筑类别">
				            <Cascader 
				            	readonly 
				            	change-on-select
				            	v-model="baseInfo.ggjzlb" 
				            	:data="ggjzlbList" 
				            	placeholder="公共建筑类别"></Cascader>
				        </FormItem>
				        <FormItem label="防空地下室掩蔽面积(㎡)">
				            <InputNumber :min="0" v-model="baseInfo.fkdxsybmj"></InputNumber>
				        </FormItem>
				        <FormItem label="建成时间">
				            <DatePicker type="date" v-model="baseInfo.jssj"  placeholder="建成时间"></DatePicker>
				        </FormItem>
				        <FormItem label="合同使用年限">
				            <DatePicker type="date" v-model="baseInfo.htsynx"  placeholder="合同使用年限"></DatePicker>
				        </FormItem>
				        <FormItem label="最近修缮时间">
				            <DatePicker type="date" v-model="baseInfo.zjxssj"  placeholder="最近修缮时间"></DatePicker>
				        </FormItem>
				        <FormItem label="区域人口（万人）">
				            <InputNumber :min="0" v-model="baseInfo.qyrk"></InputNumber>
				        </FormItem>
				        <FormItem label="使用状况">
				        	<Input clearable v-model="baseInfo.syzk" placeholder="使用状况"></Input>
				        </FormItem>
				        <FormItem label="使用单位">
				        	<Input clearable v-model="baseInfo.sydw" placeholder="使用单位"></Input>
				        </FormItem>
				        <FormItem label="使用单位负责人">
				        	<Input clearable v-model="baseInfo.sydwfzr" placeholder="使用单位负责人"></Input>
				        </FormItem>
				        <FormItem label="使用单位联系电话">
				        	<Input clearable v-model="baseInfo.sydwlxdh" placeholder="管理单位联系电话"></Input>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getRfgcBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { rfgcmc, bh, dz, gsgldw, ggjzlb, fkdxsybmj, qyrk, zrry, zrrlxdh, jssj, zjxssj, sydw, sydwfzr, sydwlxdh, htsynx, txxx } = this.form
					this.baseInfo = { rfgcmc, bh, dz, gsgldw, zrry, zrrlxdh, sydw, sydwfzr, sydwlxdh, htsynx }
					this.baseInfo.fkdxsybmj = fkdxsybmj ? Number(fkdxsybmj) : 0
					this.baseInfo.qyrk = qyrk ? Number(qyrk) : 0
					this.baseInfo.jssj = jssj ? new Date(jssj) : ''
					this.baseInfo.zjxssj = zjxssj ? new Date(zjxssj) : ''
					this.baseInfo.txxx = txxx ? JSON.parse(txxx) : []
					this.baseInfo.ggjzlb = ggjzlb ? ggjzlb.split('-') : []
			...
			async submit() {
				let params = {
					...
					ggjzlb: this.baseInfo.ggjzlb.join('-'),
					txxx: JSON.stringify(this.baseInfo.txxx),
					jssj: this.baseInfo.jssj ? getDate(new Date(this.baseInfo.jssj).getTime(), 'date') : '',
					zjxssj: this.baseInfo.zjxssj ? getDate(new Date(this.baseInfo.zjxssj).getTime(), 'date') : '',
				...
				let { status_code, message } = await api.addRfgcBase(params);
				if(status_code == 200) {
