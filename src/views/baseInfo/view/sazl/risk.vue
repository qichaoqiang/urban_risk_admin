<template>
	<div class="page">
			...
			<Row type="flex" justify="center">
				<Col span="22">
					...
					<Tabs value="name1">
				        ...
				        <TabPane label="液氨制冷系统" name="name2">
				        	<part-title text="液氨制冷系统" :btns="['add']" @add="openYdzlxtModel"></part-title>
							<Table :columns="ydzlxtColumns" :data="ydzlxtData">
								<template slot-scope="{ row }" slot="ksldzz">
									<span>{{ksldzzList[row.ksldzz].name}}</span>
								</template>
								<template slot-scope="{ row }" slot="action">
						            <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px" @click="editYdzlxtModel(row)">编辑</Button>
						            <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeYdzlxt(row)">
								        <Button type="error" size="small" ghost>删除</Button>
								    </Poptip>
						        </template>
							</Table>
							<Row type="flex" justify="end">
								<Page
				                    size="small"
				                    style="margin-top: 10px"
				                    :page-size="ydzlxtPage.pageSize"
				                    :total="ydzlxtPage.totalRow"
				                    show-elevator
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeYdzlxtPage"
				                    @on-page-size-change="handleChangeYdzlxtPageSize"
				                />
							</Row>
				        </TabPane>
				        <TabPane label="重大危险源" name="name3">
				        	<Form :disabled="isDisEditInfo" :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}液氨制冷系统`" v-model="showYdzlxtModel" @on-visible-change="ydzlxtModelChange">
			<div>
				<Form :model="ydzlxtForm" label-position="left" :label-width="140">
					<FormItem label="名称">
			        	<Input clearable v-model="ydzlxtForm.mc"></Input>
			        </FormItem>
			        <FormItem label="快速冷冻装置">
			        	<Select clearable v-model="ydzlxtForm.ksldzz" placeholder="请选择">
			                <Option v-for="item in ksldzzList" :key="item.value" :value="item.value">{{item.name}}</Option>
			            </Select>
			        </FormItem>
			        <FormItem label="热氨融霜">
			        	<Select clearable v-model="ydzlxtForm.rars" placeholder="请选择">
			                <Option v-for="item in rarsList" :key="item" :value="item">{{item}}</Option>
			            </Select>
			        </FormItem>
			        <FormItem label="贮氨器容积（立方）">
			        	<InputNumber :min="0" v-model="ydzlxtForm.zaqrjlf"></InputNumber>
			        </FormItem>
			        <FormItem label="投用时间">
			            <DatePicker type="date" v-model="ydzlxtForm.tysj"  placeholder="请选择"></DatePicker>
			        </FormItem>
			        <FormItem label="当前状态">
			            <Select clearable v-model="ydzlxtForm.dqzt" placeholder="请选择">
			                <Option v-for="item in dqztList" :key="item" :value="item">{{item}}</Option>
			            </Select>
			        </FormItem>
				</Form>
				<div>
					<part-title text="贮氨器" :btns="['add']" @add="openZaqModel"></part-title>
					<Table :columns="zaqColumns" :data="zaqData">
						<template slot-scope="{ row, index }" slot="sl">
							<span>{{row.sl}}{{row.sldw}}</span>
						</template>
						<template slot-scope="{ row, index }" slot="action">
				            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editZaqModel(row, index)">编辑</Button>
				            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeZaq(index)">
						        <Button type="error" size="small" ghost>删除</Button>
						    </Poptip>
				        </template>
					</Table>
			...
			<div slot="footer">
	            <!-- <Button type="text" size="large" @click="showWhModel = false">取消</Button> -->
		        <Button type="primary" size="large" @click="saveYdzlxt">保存</Button>
	        </div>
		</Modal>

		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}贮氨器`" v-model="showZaqModel" @on-visible-change="zaqModelChange">
			<div>
				<Form :model="zaqForm" label-position="left" :label-width="140">
					<FormItem label="规格型号">
						<Input clearable v-model="zaqForm.ggxh"></Input>
			        </FormItem>
			        <FormItem label="容积（m³）">
			        	<InputNumber :min="0" v-model="zaqForm.rj"></InputNumber>
			        </FormItem>
			        <FormItem label="数量">
			        	<InputNumber :min="0" v-model="zaqForm.sl"></InputNumber>
			        </FormItem>
			        <FormItem label="工作压力(Kpa)">
			        	<InputNumber :min="0" v-model="zaqForm.gzyl"></InputNumber>
			        </FormItem>
			        <FormItem label="工作温度(℃)">
			        	<InputNumber :min="0" v-model="zaqForm.gzwd"></InputNumber>
			        </FormItem>
			        <FormItem label="备注">
						<Input clearable type="textarea" v-model="zaqForm.bz"></Input>
			        </FormItem>
				</Form>
			</div>
			<div slot="footer">
	            <!-- <Button type="text" size="large" @click="showWhModel = false">取消</Button> -->
		        <Button type="primary" size="large" @click="saveZaq">保存</Button>
	        </div>
		</Modal>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				let ydzlxtParams = {
					gkdx_id: this.gkdx_id,
					per_page: this.ydzlxtPage.pageSize,
					page: this.ydzlxtPage.pageIndex,
				}
				let rimParams = {
					gkdx_id: this.gkdx_id,
				...
				Promise.all([api.getWhsbfcList(whParams), api.getYdzlxtList(ydzlxtParams), api.getRimList(rimParams)]).then((result) => {
				  	let whRes = result[0], ydzlxtRes = result[1], rimRes = result[2]
				  	...
				  	if(ydzlxtRes.status_code == 200) {
				  		this.ydzlxtData = ydzlxtRes.data.data.filter(item => item.gkdx_id == this.gkdx_id)
				  		this.ydzlxtPage.totalRow = ydzlxtRes.data.total
				  	}
				...
				let { status_code, data, message } = await api.getSazlBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
			...
			async saveInfo() {
				...
				let { status_code, message } = await api.addSazlBase(params);
				if(status_code == 200) {
					this.$Message.success('保存成功')
				}
			},
			...
			handleChangeYdzlxtPage(val) {
				this.ydzlxtPage.pageIndex = val
				this.getYdzlxtList()
			},
			handleChangeYdzlxtPageSize(val) {
				this.ydzlxtPage.pageSize = val
				this.getYdzlxtList()
			},
			async getYdzlxtList() {
				let params = {
				...
				let { status_code, data } = await api.getYdzlxtList(params)
				if(status_code == 200) {
					this.ydzlxtData = data.data
					this.ydzlxtPage.totalRow = data.total
				}
			},
			async openYdzlxtModel() {
				this.modeType = 1;
				this.showYdzlxtModel = true
			},
			async editYdzlxtModel(row) {
				this.ydzlxtForm = {
					mc: row.mc,
					ksldzz: row.ksldzz,
					rars: row.rars,
					zaqrjlf: row.zaqrjlf ? Number(row.zaqrjlf) : 0,
					tysj: row.tysj ? new Date(row.tysj) : '',
					dqzt: row.dqzt
				}
				this.id = row.id
				this.ydzlxt_id = row.id
				this.zaqData = row.zaqxq ? JSON.parse(row.zaqxq) : []
				this.modeType = 2;
				this.showYdzlxtModel = true
			},
			ydzlxtModelChange(status) {
				if(!status) {
					this.ydzlxtForm = {
						mc: '',
						ksldzz: '',
			...
			async removeYdzlxt(row) {
				let { status_code } = await api.deleteYdzlxtInfo(row.id)
				status_code == 200 && this.$Message.success('删除成功')
				this.getYdzlxtList()
			},
			async saveYdzlxt() {
				let params = {
					...this.ydzlxtForm,
					tysj: this.ydzlxtForm.tysj ? getDate(new Date(this.ydzlxtForm.tysj).getTime(), 'date') : '',
					zaqxq: this.zaqData.length > 0 ? JSON.stringify(this.zaqData) : '',
					gkdx_id: this.gkdx_id
				}
				...
				let { status_code, message } = await api.addYdzlxtInfo(params);
				if(status_code == 200) {
					this.$Message.success(message)
					this.showYdzlxtModel = false
					this.getYdzlxtList()
				}
			},
			...
			async editCcssModel(row) {
				...
				this.id = row.id
				this.ccss_id = row.id
				this.zaqData = row.wxhxp ? JSON.parse(row.wxhxp) : []
				this.modeType = 2;
				this.showCcssModel = true
				this.getGqscList();
			},
			...
			async saveCcss() {
				let params = {
					...
					wxhxp: this.zaqData.length > 0 ? JSON.stringify(this.zaqData) : '',
			...
			openZaqModel() {
				this.getWhsbfcList_()
				this.modeType2 = 1;
				this.showZaqModel = true
			},
			editZaqModel(row, index) {
				this.getWhsbfcList_()
				this.zaqForm = {
					ggxh: row.ggxh,
					rj: row.rj ? Number(row.rj) : 0,
					sl: row.sl ? Number(row.sl) : 0,
					gzyl: row.gzyl ? Number(row.gzyl) : 0,
					gzwd: row.gzwd ? Number(row.gzwd) : 0,
					bz: row.bz
				}
				this.zaqId = index
				this.modeType2 = 2;
			...
			async saveZaq() {
				if(this.modeType2 == 2) {
					this.zaqData.splice(this.zaqId, 1, this.zaqForm)
				}else {
					this.zaqData.push(this.zaqForm)
				}
				this.showZaqModel = false
			},
			removeZaq(index) {
				this.zaqData.splice(index, 1)
			},
