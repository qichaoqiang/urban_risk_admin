		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" disabled hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="空间类别">
				        	<Select clearable v-model="baseInfo.kjlb" placeholder="空间类别">
				                <Option v-for="item in kjlbList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="指挥场所位置">
				        	<Input clearable v-model="baseInfo.zhcswz" placeholder="指挥场所位置"></Input>
				        </FormItem>
				        <FormItem label="疏散场所位置">
				        	<Input clearable v-model="baseInfo.sscswz" placeholder="疏散场所位置"></Input>
				        </FormItem>
				        <FormItem label="疏散场所面积（㎡）">
				            <InputNumber :min="0" v-model="baseInfo.sscsmj"></InputNumber>
				        </FormItem>
				        <FormItem label="可容纳人数">
				            <InputNumber :min="0" v-model="baseInfo.krnrs"></InputNumber>
				        </FormItem>
				        <FormItem label="物资储备极限支撑能力">
				            <InputNumber :min="0" v-model="baseInfo.wzcbjxzcnl"></InputNumber>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" label-position="left" :label-width="140" disabled style="width: 600px">
				        <FormItem label="责任人员">
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getSsjdBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { ssjdmc, bh, dz, jwdfw, gsgldw, zhcswz, sscswz, yjwzcbkwz, kjlb, sscsmj, krnrs, wzcbjxzcnl, zrry, zrrlxdh, jssj, wzpzb } = this.form
					this.baseInfo = { ssjdmc, bh, dz, jwdfw, gsgldw, zhcswz, sscswz, yjwzcbkwz, kjlb, zrry, zrrlxdh }
					this.baseInfo.sscsmj = sscsmj ? Number(sscsmj) : 0
					this.baseInfo.krnrs = krnrs ? Number(krnrs) : 0
					this.baseInfo.wzcbjxzcnl = wzcbjxzcnl ? Number(wzcbjxzcnl) : 0
					this.baseInfo.jssj = jssj ? new Date(jssj) : ''
					this.baseInfo.wzpzb = wzpzb ? JSON.parse(wzpzb) : []
			...
			async submit() {
				let params = {
					...
					wzpzb: JSON.stringify(this.baseInfo.wzpzb),
				...
				let { status_code, message } = await api.addSsjdBase(params);
				if(status_code == 200) {
