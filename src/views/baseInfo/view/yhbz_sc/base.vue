			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="包装工人数">
				            <InputNumber :min="0" v-model="baseInfo.bzgrs"></InputNumber>
				        </FormItem>
				        <FormItem label="行业门类">
				        	<Cascader 
				        		clearable 
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getYhbzscBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, zgrs, glrysl, hyml, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, scdz, lngAndLat, qyfw, zdmj, jzmj, tcsj, bzgrs, jyfw } = this.form
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, scdz, qyfw, jyfw }
					this.baseInfo.tcsj = tcsj ? new Date(tcsj) : '',
					this.baseInfo.zgrs = zgrs ? Number(zgrs) : 0
					this.baseInfo.glrysl = glrysl ? Number(glrysl) : 0
					this.baseInfo.bzgrs = bzgrs ? Number(bzgrs) : 0
			...
			async nextStep() {
				...
				let { status_code, message } = await api.addYhbzscBase(params);
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getYhbzscBase()
						if(status_code == 200) {
