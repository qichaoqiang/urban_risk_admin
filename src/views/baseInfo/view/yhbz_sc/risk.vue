<template>
	<div class="page">
				...
				<Col span="22">
					...
					<Tabs value="name1">
				        <TabPane label="产品信息" name="name1">
				        	<part-title text="产品信息" ref="xxx" :btns="['add']" @add="openCpyhbzscModel"></part-title>
							<Table :columns="cpyhbzscColumns" :data="cpyhbzscData">
								<template slot-scope="{ row }" slot="scgm">
									<span>{{row.scgm}}{{row.scgmdw}}</span>
								</template>
								<template slot-scope="{ row }" slot="action">
						            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editCpyhbzscModel(row)">编辑</Button>
						            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeCpyhbzsc(row)">
							...
							<Row type="flex" justify="end">
								<Page
				                    ...
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeCpyhbzscPage"
				                    @on-page-size-change="handleChangeCpyhbzscPageSize"
				                />
							</Row>
				        </TabPane>
				        <TabPane label="仓库信息" name="name3">
							...
							<Table :columns="ckyhbzscColumns" :data="ckyhbzscData">
								...
								<template slot-scope="{ row }" slot="action">
						            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editCkyhbzModel(row)">编辑</Button>
						            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeCkyhbz(row)">
								        <Button type="error" size="small" ghost>删除</Button>
								    </Poptip>
						        </template>
							</Table>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}产品信息`" v-model="showCpyhbzscModel" @on-visible-change="cpyhbzscModelChange">
			<div>
				<Form :model="cpyhbzscForm" label-position="left" :label-width="120">
					<FormItem label="产品类别">
			        ...
			        <FormItem label="生产规模">
			        	<Row type="flex" align="middle">
	            			<Col>
	            				<InputNumber :min="0" v-model="cpyhbzscForm.scgm"></InputNumber>
	            			</Col>
	            			<Col>
	            				<Select clearable v-model="cpyhbzscForm.scgmdw" placeholder="单位">
					                <Option v-for="item in scgmdwList" :key="item" :value="item">{{item}}</Option>
					            </Select>
	            			</Col>
	            		</Row>
			        </FormItem>
			        <FormItem label="使用药量">
			        	<Row type="flex" align="middle">
	            			<Col>
	            				<InputNumber :min="0" v-model="cpyhbzscForm.syyl"></InputNumber>
	            			</Col>
	            			<Col>
	            				<Select clearable v-model="cpyhbzscForm.syyldw" placeholder="单位">
					                <Option v-for="item in syyldwList" :key="item" :value="item">{{item}}</Option>
					            </Select>
	            			</Col>
	            		</Row>
			        </FormItem>
			...
			<div slot="footer">
	            <!-- <Button type="text" size="large" @click="showCpyhbzscModel = false">取消</Button> -->
		        <Button type="primary" size="large" @click="saveCpyhbzsc">保存</Button>
	        </div>
		</Modal>
	...
	export default {
		...
		data() {
			return {
				...
				showAreaModel: false,
				showLngModel: false,
				showCpyhbzscModel: false,
				showCpyhbzscRiskModel: false,
		...
		methods: {
			getBaseInfo() {
				...
				Promise.all([api.getCpyhbzscList(cpyhbzscParams), api.getCkyhbzList(ckyhbzscParams), api.getRimList(rimParams)]).then((result) => {
			...
			async getCpyhbzscList_() {
				let params = {
					gkdx_id: this.gkdx_id,
					per_page: 1000,
					page: this.cpyhbzscPage.pageIndex,
				}
				let { status_code, data } = await api.getCpyhbzscList(params)
				if(status_code == 200) {
					this.cpyhbzscList = data.data
				}
			},
			handleChangeCpyhbzscPage(val) {
				this.cpyhbzscPage.pageIndex = val
				this.getCpyhbzscList()
			},
			handleChangeCpyhbzscPageSize(val) {
				this.cpyhbzscPage.pageSize = val
				this.getCpyhbzscList()
			},
			async getCpyhbzscList() {
				let params = {
					gkdx_id: this.gkdx_id,
					per_page: this.cpyhbzscPage.pageSize,
					page: this.cpyhbzscPage.pageIndex,
			...
			openCpyhbzscModel() {
				this.modeType = 1;
				this.showCpyhbzscModel = true
			},
			editCpyhbzscModel(row) {
				this.cpyhbzscForm = {
					cplb: row.cplb,
					cpfj: row.cpfj,
					scgm: row.scgm ? Number(row.scgm) : 0,
					scgmdw: row.scgmdw,
					syyl: row.scgm ? Number(row.syyl) : 0,
					syyldw: row.syyldw,
				}
				this.id = row.id
				this.modeType = 2;
			...
			async removeCpyhbzsc(row) {
				let { status_code } = await api.deleteCpyhbzscInfo(row.id)
			...
			async saveCpyhbzsc() {
				...
				let { status_code, message } = await api.addCpyhbzscInfo(params);
				if(status_code == 200) {
					this.$Message.success(message)
					this.showCpyhbzscModel = false
					this.getCpyhbzscList()
				}
			},
