	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, hydm, yyzt, jbr, jbrdh, jbryx, qyfzr, fzrdh, lngAndLat, zcdz, kysj, jddz, jdxj } = this.form
					this.baseInfo = { qymc, tyshxydm, hydm, yyzt, jbr, jbrdh, jbryx, qyfzr, fzrdh, lngAndLat, zcdz, jddz }
			...
			async nextStep() {
				...
				if(status_code == 200) {
					...
					if(this.$route.query.type == 2) {
						...
						let { status_code, data } = await api.getBgjdBase()
						if(status_code == 200) {
