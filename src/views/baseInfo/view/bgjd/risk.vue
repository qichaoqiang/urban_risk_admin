				...
				<Col span="22">
					...
					<Tabs value="name1">
				        <TabPane label="建筑物信息" name="name1">
				        	...
				        	<Form :disabled="isDisEditInfo" :model="baseInfo" ref="baseInfo1" :rules="baseInfoRules" hide-required-mark label-position="left" :label-width="120">
				        ...
				        <TabPane label="营业状况" name="name2">
							...
							<Form :disabled="isDisEditInfo" :model="baseInfo" ref="baseInfo2" :rules="baseInfoRules" hide-required-mark label-position="left" inline>
						        <FormItem label="房间数" prop="fjs" :label-width="118" style="margin-right: 24px">
						        	<InputNumber :min="0" v-model="baseInfo.fjs"></InputNumber>
						        </FormItem>
							</Form>
							<Table :columns="yyzkColumns" :data="yyzkData">
								<template slot-scope="{ row }" slot="action">
						            <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px" @click="editYyzkModel(row)">编辑</Button>
						            <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeYyzk(row)">
							...
							<Row type="flex" justify="end">
								<Page
				                    ...
				                    :page-size="yyzkPage.pageSize"
				                    :total="yyzkPage.totalRow"
				                    show-elevator
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeYyzkPage"
				                    @on-page-size-change="handleChangeYyzkPageSize"
				                />
							</Row>
							<Row v-show="!isDisEditInfo" type="flex" justify="center" style="margin-top: 24px">
								<Col>
		...
		methods: {
			async getBaseInfo() {
				...
				let yyzkParams = {
					...
					per_page: this.yyzkPage.pageSize,
					page: this.yyzkPage.pageIndex,
				}
				Promise.all([api.getYyzkList(yyzkParams)]).then((result) => {
				  	let yyzkRes = result[0]
				  	if(yyzkRes.status_code == 200) {
				  		this.yyzkData = yyzkRes.data.data.filter(item => item.gkdx_id == this.gkdx_id)
				  		this.yyzkPage.totalRow = yyzkRes.data.total
				  	}
				  	this.loading = false   
				}).catch((error) => {
				  	console.log(error)
			...
			async getYyzkList_() {
				...
				let { status_code, data } = await api.getYyzkList(params)
				if(status_code == 200) {
					this.yyzkList = data.data
				}
			},
			handleChangeYyzkPage(val) {
				this.yyzkPage.pageIndex = val
				this.getYyzkList()
			},
			handleChangeYyzkPageSize(val) {
				this.yyzkPage.pageSize = val
				this.getYyzkList()
			},
			async getYyzkList() {
				let params = {
				...
				let { status_code, data } = await api.getYyzkList(params)
				if(status_code == 200) {
					this.yyzkData = data.data.filter(item => item.gkdx_id == this.gkdx_id)
					this.yyzkPage.totalRow = data.total
				}
			},
			openYyzkModel() {
				this.modeType = 1;
				this.showYyzkModel = true
			},
			editYyzkModel(row) {
				this.yyzkForm = {
					jyzpjrkfsl: row.jyzpjrkfsl ? Number(row.jyzpjrkfsl) : 0,
			...
			async removeYyzk(row) {
				let { status_code } = await api.deleteYyzkInfo(row.id)
