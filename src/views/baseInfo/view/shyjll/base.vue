  ...
  <div class="page">
    ...
    <div v-show="step == 1">
      <Row type="flex" justify="center">
        ...
        <Form disabled :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140"
          style="width: 600px">
          <FormItem label="队伍名称">
            {{baseInfo.dwmc}}
          </FormItem>
          ...
          <FormItem label="登记机构" prop="dwlsdw">
            <Input clearable v-model="baseInfo.djjg" placeholder="登记机构"></Input>
          </FormItem>
          ...
          <!-- <FormItem label="场地范围">
            <div @click.stop="openAreaModal">
              <Input readonly v-model="baseInfo.jwdfw" icon="md-pin" placeholder="场地范围" />
            </div>
          </FormItem> -->
          <FormItem label="所属辖区" prop="quyu">
            <Cascader readonly change-on-select v-model="baseInfo.quyu" :data="areaList" :load-data="loadArea"
          ...
          <FormItem label="值班电话">
            <Input clearable v-model="baseInfo.zbdh" placeholder="值班电话"></Input>
          </FormItem>
          <FormItem label="统一社会信用代码">
            <Input clearable v-model="baseInfo.tyshxydm" placeholder="统一社会信用代码"></Input>
          </FormItem>
          <FormItem label="队伍驻地">
            <Input clearable v-model="baseInfo.dwzd" placeholder="队伍驻地"></Input>
          </FormItem>
          <FormItem label="民政注册代码">
            <Input clearable v-model="baseInfo.mzzcdm" placeholder="民政注册代码"></Input>
          </FormItem>
          <FormItem label="民政评估等级">
            <Input clearable v-model="baseInfo.mzpgdj" placeholder="民政评估等级"></Input>
          </FormItem>
          <FormItem label="评估主管单位">
            <Input clearable v-model="baseInfo.pgzgdw" placeholder="评估主管单位"></Input>
          </FormItem>
          <FormItem label="队伍能力类型">
            <Input clearable v-model="baseInfo.dwnllx" placeholder="队伍能力类型"></Input>
          </FormItem>
        </Form>
        </Col>
      </Row>
        ...
        <Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
          <FormItem label="负责人">
            <Row type="flex" :gutter="20">
              <Col span="8">
              <Input clearable v-model="baseInfo.fzrxm" placeholder="姓名"></Input>
              </Col>
              <Col span="16">
          ...
          <FormItem label="应急联系人">
            <Row type="flex" :gutter="20">
              <Col span="8">
              <Input clearable v-model="baseInfo.yjlxrxm" placeholder="姓名"></Input>
              </Col>
              <Col span="16">
              <Input clearable v-model="baseInfo.yjlxrdh" placeholder="电话"></Input>
              </Col>
            </Row>
          </FormItem>
          <FormItem label="主管单位">
            <Row type="flex" :gutter="20">
              <Col span="8">
              <Input clearable v-model="baseInfo.zgdwmc" placeholder="名称"></Input>
              </Col>
              <Col span="16">
              <Input clearable v-model="baseInfo.zgdwlxdh" placeholder="联系电话"></Input>
              </Col>
              <Col span="16">
              <Input clearable v-model="baseInfo.zgdwlxr" placeholder="联系人"></Input>
              </Col>
            </Row>
          </FormItem>
        </Form>
      ...
      <!-- <Row type="flex" justify="center">
        <Col>
        <part-title text="物资配置表" :btns="['add']" @add="openWzpzbModel"></part-title>
        <Table :columns="wzpzbColumns" :data="baseInfo.wzpzb" style="width: 600px">
          <template slot-scope="{ row, index }" slot="action">
            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editWzpzbModel(row, index)">编辑</Button>
            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeWzpzb(index)">
    ...
    methods: {
      async getBaseInfo() {
        let {
          ...
          data,
          message
        } = await api.getShyjllBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          let {
            dwmc, zcdz, djjg, zbdh, tyshxydm, dwzd, fzrxm, fzrdh, yjlxrxm, yjlxrdh, zgdwmc, zgdwlxdh, zgdwlxr, mzzcdm, mzpgdj, pgzgdw, dwnllx, clsj
          } = this.form
          this.baseInfo = {
            dwmc, zcdz, djjg, zbdh, tyshxydm, dwzd, fzrxm, fzrdh, yjlxrxm, yjlxrdh, zgdwmc, zgdwlxdh, zgdwlxr, mzzcdm, mzpgdj, pgzgdw, dwnllx
          }
      ...
      async submit() {
        ...
        let {
          status_code,
          message
        } = await api.addShyjllBase(params);
        if (status_code == 200) {
      ...
      async getWhsbfcList() {
        ...
        let {
          status_code,
          data
        } = await api.getWhsbfcList(params)
        if (status_code == 200) {
      ...
      async removeWh(row) {
        let {
          status_code
        } = await api.deleteWhsbfcInfo(row.id)
      ...
      async saveWh() {
        ...
        let {
          status_code,
          message
        } = await api.addWhsbfcInfo(params);
        if (status_code == 200) {
      ...
      async getWhRiskList() {
        ...
        let {
          status_code,
          data
        } = await api.getWhRiskList(params)
        if (status_code == 200) {
      ...
      async removeWhRisk(row) {
        let {
          status_code
        } = await api.deleteWhRiskInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getWhRiskList()
      },
      async saveWhRisk() {
        ...
        let {
          status_code,
          message
        } = await api.addWhRiskInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
      ...
      async getSbfcgyList() {
        ...
        let {
          status_code,
          data
        } = await api.getSbfcgyList(params)
        if (status_code == 200) {
      ...
      async removeSbfcgy(row) {
        let {
          status_code
        } = await api.deleteSbfcgyInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getSbfcgyList()
      },
      async saveSbfcgy() {
        ...
        let {
          status_code,
          message
        } = await api.addSbfcgyInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
      ...
      async getDeviceList() {
        ...
        let {
          status_code,
          data
        } = await api.getDeviceList(params)
        if (status_code == 200) {
      ...
      async saveDevice() {
        ...
        let {
          status_code,
          message
        } = await api.addDeviceInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
      ...
      async removeDevice(row) {
        let {
          status_code
        } = await api.deleteDeviceInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getDeviceList()
      },
      ...
      async getMainRiskList() {
        ...
        let {
          status_code,
          data
        } = await api.getMainRiskList(params)
        if (status_code == 200) {
      ...
      async removeMainRisk(row) {
        let {
          status_code
        } = await api.deleteMainRiskInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getMainRiskList()
      },
      async saveMainRisk() {
        ...
        let {
          status_code,
          message
        } = await api.addMainRiskInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
