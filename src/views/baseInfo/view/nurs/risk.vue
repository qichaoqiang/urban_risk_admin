<template>
  <div class="page">
          ...
          <TabPane label="人员信息" name="name1">
            ...
            <Row type="flex" justify="end">
              ...
              <Table :columns="szdxColumns" :data="szdxData">
                ...
                <template slot-scope="{ row }" slot="action">
                  <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                    @click="editSzdxModel(row)">编辑</Button>
                  <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                    @on-ok="removeSzdx(row)">
                    <Button type="error" size="small" ghost>删除</Button>
                  </Poptip>
                </template>
              </Table>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}工作负荷`" v-model="showGzfhModel"
      @on-visible-change="gzfhModelChange">
      <div>
        <Form :model="gzfhForm" ref="gzfh" :rules="gzfhRules" hide-required-mark label-position="left"
          :label-width="120">
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}收住对象`" v-model="showSzdxModel"
      @on-visible-change="szdxModelChange">
      <div>
        <Form :model="szdxForm" ref="szdx" :rules="szdxRules" hide-required-mark label-position="left"
          :label-width="120">
  ...
  export default {
    ...
    methods: {
      async getBaseInfo() {
        ...
        Promise.all([api.getGzfhItemList(gzfhParams), api.getSzdxItemList(szdxParams), api.getNursBase(this.gkdx_id)])
          .then((result) => {
            let gzfhRes = result[0],
              szdxRes = result[1];
      ...
      async getGzfhItemList() {
        ...
        let {
          status_code,
          data
        } = await api.getGzfhItemList(params)
        if (status_code == 200) {
      ...
      async removeGzfh(row) {
        let {
          status_code
        } = await api.deleteGzfhItem(row.id)
      ...
      async getSzdxItemList() {
        ...
        let {
          status_code,
          data
        } = await api.getSzdxItemList(params)
        if (status_code == 200) {
      ...
      async removeSzdx(row) {
        let {
          status_code
        } = await api.deleteSzdxItem(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getSzdxItemList()
      },
      async saveSzdx() {
    ...
    created() {
      ...
      if (this.isDisEditInfo) {
        this.gzfhColumns.pop()
        this.szdxColumns.pop()
      }
    },
    mounted() {
