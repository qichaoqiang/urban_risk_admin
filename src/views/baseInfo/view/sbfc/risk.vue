<template>
  <div class="page">
        ...
        <Tabs value="name1">
          <TabPane label="危险化学品" name="name1">
            ...
            <Table :columns="whColumns" :data="whData">
              ...
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editWhModel(row)">编辑</Button>
            ...
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="whPage.pageSize" :total="whPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeWhPage"
          ...
          <TabPane label="危化品重大危险源" name="name2">
            ...
            <Table :columns="whRiskColumns" :data="whRiskData">
              ...
              <template slot-scope="{ row }" slot="action">
                <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                  @click="editWhRiskModel(row)">编辑</Button>

                <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                  @on-ok="removeWhRisk(row)">
                  <Button type="error" size="small" ghost>删除</Button>
                </Poptip>
              </template>
            </Table>
            <Row type="flex" justify="end">
              <Page size="small" style="margin-top: 10px" :page-size="whRiskPage.pageSize" :total="whRiskPage.totalRow"
                show-elevator show-total show-sizer @on-change="handleChangeWhRiskPage"
          ...
          <TabPane label="工艺信息" name="name3">
            ...
            <Row type="flex" justify="end">
              ...
              <Table :columns="sbfcgyColumns" :data="sbfcgyData">
                <template slot-scope="{ row }" slot="action">
                  <Button v-show="!isDisEditInfo" type="primary" size="small" ghost style="margin-right: 5px"
                    @click="editSbfcgyModel(row)">编辑</Button>
                  <Poptip v-show="!isDisEditInfo" confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？"
                    @on-ok="removeSbfcgy(row)">
                    <Button type="error" size="small" ghost>删除</Button>
                  </Poptip>
                </template>
              </Table>
              <Row type="flex" justify="end">
                <Page size="small" style="margin-top: 10px" :page-size="sbfcgyPage.pageSize"
                  :total="sbfcgyPage.totalRow" show-elevator show-total show-sizer @on-change="handleChangeSbfcgyPage"
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}危险化学品`" v-model="showWhModel"
      @on-visible-change="whModelChange">
      <div>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}危化品重大危险源`" v-model="showWhRiskModel"
      @on-visible-change="whRiskModelChange">
      <div>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}涉爆粉尘工艺`" v-model="showSbfcgyModel"
      @on-visible-change="sbfcgyModelChange">
      <div>
        <Form :model="sbfcgyForm" label-position="left" :label-width="120">
          ...
          <FormItem label="粉尘类型">
            <Cascader clearable v-model="sbfcgyForm.fclx" :data="fclxList" placeholder="粉尘类型"></Cascader>
          </FormItem>
          <FormItem label="作业人数">
        ...
        <div v-show="modeType == 2 && sbfcgyForm.ccss == '有'">
          ...
          <Table v-if="sbfcgyForm.ccss == '有'" :columns="deviceColumns" :data="deviceData">
            ...
            <template slot-scope="{ row }" slot="action">
              <Button type="primary" size="small" ghost style="margin-right: 5px"
                @click="editDeviceModel(row)">编辑</Button>
          ...
          <Row type="flex" justify="end">
            <Page size="small" style="margin-top: 10px" :page-size="devicePage.pageSize" :total="devicePage.totalRow"
              show-elevator show-total show-sizer @on-change="handleChangeDevicePage"
    ...
    <Modal width="820" :title="`${modeType2 == 1 ? '新增' : '编辑'}除尘设施`" v-model="showDeviceModel"
      @on-visible-change="deviceModelChange">
      <div>
    ...
    <Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}重点监管危险工艺`" v-model="showMainRiskModel"
      @on-visible-change="mainRiskModelChange">
      <div>
  ...
  export default {
    ...
    methods: {
      getBaseInfo() {
        ...
        Promise.all([api.getWhsbfcList(whParams), api.getWhRiskList(whRiskParams), api.getSbfcgyList(sbfcgyParams), api
          .getMainRiskList(mainRiskParams), api.getRimList(rimParams)
        ]).then((result) => {
          let whRes = result[0],
            whRiskRes = result[1],
            sbfcgyRes = result[2],
            mainRiskRes = result[3],
            rimRes = result[4]
      ...
      async getDevicesbfcList() {
        ...
        let {
          status_code,
          data
        } = await api.getDevicesbfcList(params)
        if (status_code == 200) {
      ...
      async saveDevice() {
        ...
        let {
          status_code,
          message
        } = await api.addDevicesbfcInfo(params);
        if (status_code == 200) {
          this.$Message.success(message)
      ...
      async removeDevice(row) {
        let {
          status_code
        } = await api.deleteDevicesbfcInfo(row.id)
        status_code == 200 && this.$Message.success('删除成功')
        this.getDevicesbfcList()
      },
    ...
    created() {
      ...
      if (this.isDisEditInfo) {
        this.whColumns.pop()
        this.whRiskColumns.pop()
        this.sbfcgyColumns.pop()
        this.rimColumns.pop()
        this.deviceColumns.pop()
      }
    },
    mounted() {
