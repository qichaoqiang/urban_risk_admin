<template>
	<div class="page">
					...
					<Tabs value="name1">
				        <TabPane label="场所信息" name="name1">
				        	...
				        	<Form :disabled="isDisEditInfo" :model="baseInfo" label-position="left" inline>
					        	...
					        	<FormItem label="固定座位数" :label-width="90" style="margin-right: 24px">
						        	<InputNumber :min="0" v-model="baseInfo.gdzws"></InputNumber>
						        </FormItem>
						        <FormItem label="场所可容纳人数" :label-width="118" style="margin-right: 24px">
						        	<InputNumber :min="0" v-model="baseInfo.cskrnrs"></InputNumber>
						        </FormItem>
						        <FormItem label="固定包厢数" :label-width="90" style="margin-right: 24px">
						        	<InputNumber :min="0" v-model="baseInfo.gdbxs"></InputNumber>
						        </FormItem>
						    </Form>
				        ...
				        <TabPane label="场所危险性" name="name2">
							...
							<Form :disabled="isDisEditInfo" :model="baseInfo" label-position="left" inline>
						        ...
						        <FormItem label="火灾高危单位" :label-width="104">
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}人流特征`" v-model="showRltzModel" @on-visible-change="rltzModelChange">
			<div>
				...
				rltzColumns: [
					{
				        ...
				        align: "center",
				        render: (h, params) => {
				            return h('span',params.index + (this.rltzPage.pageIndex- 1) * this.rltzPage.pageSize + 1);
				        },
                    }, {
                        title: '近一个月的平均日流量',
                        key: 'jygydpjrll',
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { 
						...
						gdzws: data.gdzws ? Number(data.gdzws) : 0, 
						cskrnrs: data.cskrnrs ? Number(data.cskrnrs) : 0, 
						gdbxs: data.gdbxs ? Number(data.gdbxs) : 0, 
			...
			async saveInfo1() {
				let params = {
					...
					gdzws: this.baseInfo.gdzws,
					cskrnrs: this.baseInfo.cskrnrs,
					gdbxs: this.baseInfo.gdbxs
				}
