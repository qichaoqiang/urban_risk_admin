<template>
	<div class="page">
				...
				<Col span="22">
					...
					<Tabs value="name1">
				        <TabPane label="管道信息" name="name1">
				        	...
				        	<Form :model="baseInfo" ref="baseInfo1" :rules="baseInfoRules1" hide-required-mark label-position="left" :label-width="220">
							...
							<Table :columns="gdxxsqColumns" :data="gdxxsqData">
								<template slot-scope="{ row }" slot="action">
						            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editGdxxsqModel(row)">编辑</Button>
						            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeGdxxsq(row)">
							...
							<Row type="flex" justify="end">
								<Page
				                    ...
				                    :page-size="gdxxsqPage.pageSize"
				                    :total="gdxxsqPage.totalRow"
				                    show-elevator
				                    show-total
				                    show-sizer
				                    @on-change="handleChangeGdxxsqPage"
				        ...
				        <TabPane label="高后果区" name="name5">
							...
							<Table :columns="ghgqColumns" :data="ghgqData">
								<template slot-scope="{ row }" slot="action">
						            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="editGhgqModel(row)">编辑</Button>
						            <Poptip confirm placement="left-end" :transfer="true" title="确认删除该条数据吗？" @on-ok="removeGhgq(row)">
								        <Button type="error" size="small" ghost>删除</Button>
								    </Poptip>
						        </template>
							</Table>
		...
		<Modal width="820" :title="`${modeType == 1 ? '新增' : '编辑'}管道信息`" v-model="showGdxxsqModel" @on-visible-change="gdxxsqModelChange">
			<div>
				<Form :model="gdxxsqForm" ref="gdxxsq" :rules="gdxxsqRules" hide-required-mark label-position="left" :label-width="130">
		...
		computed: {
			...
			gdxxsqRules() {
				return {
					...
					zcsl: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				Promise.all([api.getGdxxsqList(gdxxsqParams), api.getGhgqList(ghgqParams)]).then((result) => {
				  	let gdxxsqRes = result[0], ghgqRes = result[1]
				...
				let { status_code, data, message } = await api.getCsgxsqBase(this.gkdx_id);
				if(status_code == 0) {
					this.baseInfo = {
			...
			async saveInfo1() {
				this.$refs.baseInfo1.validate(async valid => {
                    if (valid) {
						...
						let { status_code, message } = await api.addCsgxsqBase(params);
						if(status_code == 200) {
							this.$Message.success('保存成功')
						}
                    }
			...
			async getGdxxsqList_() {
				...
				let { status_code, data } = await api.getGdxxsqList(params)
				if(status_code == 200) {
			...
			async removeGdxxsq(row) {
				let { status_code } = await api.deleteGdxxsqInfo(row.id)
			...
			async saveGdxxsq() {
				this.$refs.gdxxsq.validate(async valid => {
                    if (valid) {
						...
						let { status_code, message } = await api.addGdxxsqInfo(params);
						if(status_code == 200) {
