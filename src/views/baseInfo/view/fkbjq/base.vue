			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="经营规模">
				        	<Select clearable v-model="baseInfo.sblx" placeholder="经营规模">
				                <Option v-for="item in sblxList" :key="item" :value="item">{{item}}</Option>
				            </Select>
				        </FormItem>
				        <FormItem label="警报种类">
				        	<Input clearable v-model="baseInfo.jbzl" placeholder="警报种类"></Input>
				        </FormItem>
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getFkbjqBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { fkjbqmc, bh, dz, sblx, jbzl, gsgldw, zrry, zrrlxdh, jssj, zjxssj } = this.form
					this.baseInfo = { fkjbqmc, bh, dz, sblx, jbzl, gsgldw, zrry, zrrlxdh }
			...
			async submit() {
				...
				let { status_code, message } = await api.addFkbjqBase(params);
				if(status_code == 200) {
