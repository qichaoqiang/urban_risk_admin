    ...
    <div v-show="step == 1">
      <Row type="flex" justify="center">
        ...
        <Form disabled :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140"
          ...
          <FormItem label="类型">
            <Input clearable v-model="baseInfo.lx" placeholder="类型"></Input>
          </FormItem>
          <FormItem label="类别">
            <Input clearable v-model="baseInfo.lb" placeholder="类别"></Input>
          </FormItem>
          <FormItem label="容纳人数">
              <InputNumber :min="0" v-model="baseInfo.rnrs"></InputNumber>
          </FormItem>
          <FormItem label="面积（㎡）">
          	<InputNumber :min="0" v-model="baseInfo.mj" placeholder="面积"></InputNumber>
          </FormItem>
          <FormItem label="建成时间">
          	<DatePicker type="date" v-model="baseInfo.jcsj"  placeholder="请选择"></DatePicker>
          </FormItem>
          <FormItem label="管理单位">
        ...
        <Form disabled :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
          <FormItem label="联系人">
            <Row type="flex" :gutter="20">
              <Col span="8">
              <Input clearable v-model="baseInfo.lxr" placeholder="姓名"></Input>
              </Col>
              <Col span="16">
    ...
    methods: {
      async getBaseInfo() {
        let {
          ...
          data,
          message
        } = await api.getBncsBase(this.gkdx_id);
        if (status_code == 0) {
          this.form = data;
          let {
            csmc, dz, jcsj, cdfw, rnrs, mj, lx, lb, gldw, lxr, lxfs
          } = this.form
          this.baseInfo = {
            csmc, dz, lx, lb, gldw, lxr, lxfs
          }
          this.baseInfo.jcsj = jcsj ? new Date(jcsj) : ''
          this.baseInfo.rnrs = rnrs ? Number(rnrs) : 0
          this.baseInfo.mj = mj ? Number(mj) : 0
      ...
      async submit() {
        let params = {
          ...this.baseInfo,
          jcsj: this.baseInfo.jcsj ? getDate(new Date(this.baseInfo.jcsj).getTime(), 'date') : '',
