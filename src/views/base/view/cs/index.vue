		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="业态">
				        	<Input clearable v-model="baseInfo.yt" placeholder="业态"></Input>
				        </FormItem>
				        <FormItem label="规模">
				        	<Input clearable v-model="baseInfo.gm" placeholder="规模"></Input>
				        </FormItem>
                <FormItem label="开发单位">
                	<Input clearable v-model="baseInfo.kfdw" placeholder="开发单位"></Input>
                </FormItem>
                <FormItem label="运营单位">
                	<Input clearable v-model="baseInfo.yydw" placeholder="运营单位"></Input>
				        ...
				        <FormItem label="备注">
				            <Input type="textarea" :rows="4" v-model="baseInfo.bz" placeholder="备注"></Input>
				        </FormItem>
					</Form>
				</Col>
			</Row>
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="主体建筑层数">
				        	<InputNumber :min="0" v-model="baseInfo.ztjzcs"></InputNumber>
				        </FormItem>
					</Form>
				</Col>
			</Row>
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="超市负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
				        			<Input clearable v-model="baseInfo.csfzr" placeholder="姓名"></Input>
				        		</Col>
				        		<Col span="16">
				        			<Input clearable v-model="baseInfo.csfzrdh" placeholder="电话"></Input>
				        		</Col>
				        	</Row>
				        </FormItem>
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getCsBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { scmc, tyshxydm, hydm, yysj1, yysj2, kfdw, zzdjdz, zcdjdz, yydw, yt, gm, bz, jbr, jbrdh, jbryx, csfzr, csfzrdh, fgaqfzr, fgaqfzrdh, dz, lngAndLat, dyfw, jzzdmj, ztjzcs, tysj, jzlx } = this.form
					this.baseInfo = { scmc, tyshxydm, hydm, yysj1, yysj2, kfdw, zzdjdz, zcdjdz, yydw, yt, gm, bz, jbr, jbrdh, jbryx, csfzr, csfzrdh, fgaqfzr, fgaqfzrdh, dz, lngAndLat, dyfw, jzlx }
			...
			async submit() {
				...
				let { status_code, message } = await api.addCsBase(params);
				if(status_code == 200) {
