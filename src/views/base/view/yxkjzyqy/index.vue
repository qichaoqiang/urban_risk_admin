	...
	<div class="page">
		...
		<div v-show="step == 1">
			<Row type="flex" justify="center">
                ...
                <FormItem label="是否造纸、酱腌菜生产企业" :label-width="104" style="margin-right: 24px">
                  <Select clearable v-model="baseInfo.sfzzjycscqy" placeholder="请选择">
                    <Option v-for="item in sfzgyyList" :key="item.value" :value="item.value">{{item.name}}</Option>
                  </Select>
                </FormItem>
                <FormItem label="有无附属污水处理系统" :label-width="104" style="margin-right: 24px">
                  <Select clearable v-model="baseInfo.ywfswsclxt" placeholder="请选择">
                    <Option v-for="item in ywfswsclxtList" :key="item.value" :value="item.value">{{item.name}}</Option>
                  </Select>
                </FormItem>
				        <FormItem label="有限空间类别">
				        	<Input clearable v-model="baseInfo.yxkjlb" placeholder="有限空间类别"></Input>
				        </FormItem>
				        <FormItem label="可能涉及的作业内容">
				        	<Input clearable v-model="baseInfo.knsjdzynr" placeholder="可能涉及的作业内容"></Input>
				        </FormItem>
                <FormItem label="有限空间作业所在位置">
                	<Input clearable v-model="baseInfo.yxkjzyszwz" placeholder="有限空间作业所在位置"></Input>
                </FormItem>
                <FormItem label="最大涉及作业人数">
                	<InputNumber :min="0" v-model="baseInfo.zdsjzyrs"></InputNumber>
                </FormItem>
                <FormItem label="危险有害因素">
                	<Input clearable v-model="baseInfo.wxyhys" placeholder="危险有害因素"></Input>
                </FormItem>
                <FormItem label="空间容积（立方）">
                	<Input clearable v-model="baseInfo.kjrjlf" placeholder="空间容积（立方）"></Input>
                </FormItem>
                <FormItem label="隐患信息">
                	<Input clearable v-model="baseInfo.yhxx" placeholder="隐患信息"></Input>
                </FormItem>
				        <FormItem label="行业门类">
				        	<Cascader
				        		clearable
					...
					<Form :model="baseInfo" label-position="left" :label-width="140" style="width: 600px">
						<FormItem label="负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
				        			<Input clearable v-model="baseInfo.fzr" placeholder="姓名"></Input>
				        		</Col>
				        		<Col span="16">
	...
	export default {
		...
		data() {
			return {
				...
				yyztList: ['在运行', '封闭升级', '休业'],
				jzlxList: ['单层建筑', '一类高层', '二类高层'],
				sfzgyyList: [
					{
						value: 0,
						name: '否'
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getYxkjzyqyBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, hydm, qydz, yysj2, kfdw, yxkjlb, knsjdzynr, yxkjzyszwz, yt, wxyhys, kjrjlf, yhxx, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat, sfzzjycscqy, ywfswsclxt, zdsjzyrs } = this.form
					this.baseInfo = { qymc, tyshxydm, hydm, qydz, yysj2, kfdw, yxkjlb, knsjdzynr, yxkjzyszwz, yt, wxyhys, kjrjlf, yhxx, jbr, jbrdh, jbryx, fzr, fzrdh, lngAndLat }
					this.baseInfo.sfzzjycscqy = sfzzjycscqy ? Number(sfzzjycscqy) : 0
					this.baseInfo.ywfswsclxt = ywfswsclxt ? Number(ywfswsclxt) : 0
					this.baseInfo.zdsjzyrs = zdsjzyrs ? Number(zdsjzyrs) : 0
			...
			async submit() {
				...
				let { status_code, message } = await api.addYxkjzyqyBase(params);
				if(status_code == 200) {
