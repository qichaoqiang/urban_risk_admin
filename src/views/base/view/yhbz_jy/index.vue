			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="仓储地址" prop="ccdz">
				        	<Input clearable v-model="baseInfo.ccdz" placeholder="仓储地址"></Input>
				        </FormItem>
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng id="lng_box" :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.ccdz"></lng>
				        </FormItem>
				        <FormItem label="仓储范围">
				        	<div @click.stop="openAreaModal">
			        			<Input 
			        				readonly 
			        				v-model="baseInfo.ccfw" 
			        				icon="md-pin" 
			        				placeholder="仓储范围" />
			        		</div>
				        </FormItem>
				        <FormItem label="投产时间">
				        	<DatePicker type="date" v-model="baseInfo.tcsj"  placeholder="请选择"></DatePicker>
				        </FormItem>
...
<script>
	...
	export default {
		...
		computed: {
			rules() {
				return {
                	ccdz: [{ required: true, message: '请输入', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getYhbzjyBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, zgrs, glrysl, hyml, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, ccdz, lngAndLat, ccfw, zdmj, jzmj, tcsj, jyfw } = this.form
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, qyjbjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, ccdz, ccfw, jyfw }
					this.baseInfo.tcsj = tcsj ? new Date(tcsj) : '',
			...
			async submit() {
				...
				let { status_code, message } = await api.addYhbzjyBase(params);
				if(status_code == 200) {
			...
			openAreaModal() {	
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.ccfw) {
			            	let ccfw = JSON.parse(this.baseInfo.ccfw)
			            	let points = ccfw.map(item => {
			...
			openArea() {
				...
				this.polygonTool.addEventListener('draw', (e) => {
					// 获取绘制的多边形信息
					console.log(e);
					this.baseInfo.ccfw = JSON.stringify(e.currentLnglats)
				})
			},
			clearArea() {
				this.baseInfo.ccfw = ''
			...
			cancelArea() {
				...
				this.baseInfo.ccfw = this.ccfw || this.form.ccfw || '';
				this.showAreaModel = false
			},
			saveArea() {
				this.ccfw = this.baseInfo.ccfw;
				this.form.ccfw = this.baseInfo.ccfw;
				this.map.clearOverLays();
				this.map = null;
				this.polygonTool = null;
				this.showAreaModel = false
