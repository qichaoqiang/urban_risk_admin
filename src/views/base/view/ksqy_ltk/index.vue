			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="安全生产许可证" prop="aqscxkz">
				        	<Input clearable v-model="baseInfo.aqscxkz" placeholder="安全生产许可证"></Input>
				        </FormItem>
				        <FormItem label="矿山地址" prop="ksdz">
				        	<Input clearable v-model="baseInfo.ksdz" placeholder="矿山地址"></Input>
				        </FormItem>
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng id="lng_box" :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.ksdz"></lng>
				        </FormItem>
				        <FormItem label="矿区范围" prop="kqfw">
				        	<div @click.stop="openAreaModal">
			        			<Input 
			        				readonly 
			        				v-model="baseInfo.kqfw" 
			        				icon="md-pin" 
			        				placeholder="矿区范围" />
			        		</div>
				        </FormItem>
				        ...
				        <FormItem label="开采时间" prop="kcsj">
				        	<DatePicker type="date" v-model="baseInfo.kcsj"  placeholder="请选择"></DatePicker>
				        </FormItem>
				        <FormItem label="开采矿种" prop="kckz">
				        	<Input clearable v-model="baseInfo.kckz" placeholder="开采矿种"></Input>
				        </FormItem>
	...
	export default {
		...
		computed: {
			rules() {
				...
				return {
                	ksdz: [{ required: true, message: '请输入', trigger: 'change' }],
     ...
     //            	aqscxkz: [{ required: true, message: '请输入', trigger: 'change' }],
     //            	jyfw: [{ required: true, message: '请输入', trigger: 'change' }],
     //            	kqfw: [{ required: true, message: '请选择', trigger: 'change' }],
     //            	zgrs: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
     //            	glrysl: [{ required: true, type: 'number', message: '请输入', trigger: 'change' }],
     //            	kcsj: [{ required: true, type: 'date', message: '请输入', trigger: 'change' }],
     //            	kckz: [{ required: true, message: '请输入', trigger: 'change' }],
     ...
     //            	ksjk: [{ required: true, message: '请输入', trigger: 'change' }],
     ...
     //            	fgaqfzrdh: [{ required: true, message: '请输入', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hyml, hydm, aqscxkz, zgrs, glrysl, ksjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, ksdz, lngAndLat, kqfw, zdmj, kcsj, jyfw, ktfs, bpzy, swtj, dztj, sjkcsd, kckz } = this.form
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, zgrs, glrysl, ksjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, ksdz, kqfw, jyfw, ktfs, bpzy, swtj, dztj, kckz, kcsj, sjkcsd, quyu: [] }
					...
					this.baseInfo.kqfw = kqfw || ''
					this.baseInfo.ksjk = ksjk || ''
					this.baseInfo.jbr = jbr || ''
					this.baseInfo.jbrdh = jbrdh || ''
					this.baseInfo.jbryx = jbryx || ''
					this.baseInfo.qyfzr = qyfzr || ''
					this.baseInfo.qyfzrdh = qyfzrdh || ''
					this.baseInfo.fgaqfzr = fgaqfzr || ''
					this.baseInfo.fgaqfzrdh = fgaqfzrdh || ''
					this.baseInfo.aqscxkz = aqscxkz || ''
			...
			async submit() {
				let params = {
					...this.baseInfo,
					kcsj: this.baseInfo.kcsj ? getDate(new Date(this.baseInfo.kcsj).getTime(), 'date') : '',
			...
			openAreaModal() {	
				...
				this.$nextTick(() => {
		            ...
		            lo.getCurrentPosition((e) => {
			            ...
			            if(this.baseInfo.kqfw) {
			            	let kqfw = JSON.parse(this.baseInfo.kqfw)
			            	let points = kqfw.map(item => {
			...
			openArea() {
				...
				this.polygonTool.addEventListener('draw', (e) => {
					// 获取绘制的多边形信息
					console.log(e);
					this.baseInfo.kqfw = JSON.stringify(e.currentLnglats)
				})
			},
			clearArea() {
				this.baseInfo.kqfw = ''
			...
			cancelArea() {
				...
				this.baseInfo.kqfw = this.kqfw || this.form.kqfw || '';
				this.showAreaModel = false
			},
			saveArea() {
				this.kqfw = this.baseInfo.kqfw;
				this.form.kqfw = this.baseInfo.kqfw;
				this.map.clearOverLays();
				this.map = null;
				this.polygonTool = null;
				this.showAreaModel = false
