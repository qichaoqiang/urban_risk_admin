			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="企业类型" prop="qylx">
				        	<Input clearable v-model="baseInfo.qylx" placeholder="企业类型"></Input>
				        </FormItem>
				        <!-- <FormItem label="场地范围">
				        ...
				        <FormItem label="经营船舶数量">
				            <InputNumber :min="0" v-model="baseInfo.jycbsl"></InputNumber>
				        </FormItem>
				        <FormItem label="总载重（万吨）">
				            <InputNumber :min="0" v-model="baseInfo.zzz"></InputNumber>
				        </FormItem>
				        <FormItem label="船舶总功率（KW）">
				            <InputNumber :min="0" v-model="baseInfo.cbzgl"></InputNumber>
				        </FormItem>
				        <FormItem label="年货运量（万吨）">
				            <InputNumber :min="0" v-model="baseInfo.nhyl"></InputNumber>
				        </FormItem>
				        <FormItem label="主要货物种类">
				        	<Input clearable type="textarea" v-model="baseInfo.zyhwzl" placeholder="主要货物种类"></Input>
				        </FormItem>
				        <FormItem label="经营范围">
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getSshyBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { mc, jbr, jbrdh, jbryx, dz, sjqy, qylx, jycbsl, zzz, cbzgl, nhyl, zyhwzl, jyfw } = this.form
					this.baseInfo = { mc, jbr, jbrdh, jbryx, dz, sjqy, qylx, zyhwzl, jyfw }
					this.baseInfo.jycbsl = jycbsl ? Number(jycbsl) : 0
					this.baseInfo.zzz = zzz ? Number(zzz) : 0
					this.baseInfo.cbzgl = cbzgl ? Number(cbzgl) : 0
					this.baseInfo.nhyl = nhyl ? Number(nhyl) : 0
			...
			async submit() {
				...
				let { status_code, message } = await api.addSshyBase(params);
				if(status_code == 200) {
