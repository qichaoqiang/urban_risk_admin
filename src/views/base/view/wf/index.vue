			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="建筑结构">
				        	<Input clearable v-model="baseInfo.jzjg" placeholder="建筑结构"></Input>
				        </FormItem>
				        <FormItem label="建设时间">
				            <DatePicker type="date" v-model="baseInfo.jssj"  placeholder="请选择"></DatePicker>
				        </FormItem>
				        <FormItem label="建筑面积（㎡）">
				        	<InputNumber :min="0" v-model="baseInfo.jzmj"></InputNumber>
				        </FormItem>
				        <FormItem label="危房鉴定级别">
				        	<Input clearable v-model="baseInfo.wfjdjb" placeholder="危房鉴定级别"></Input>
				        </FormItem>
				        <FormItem label="现居住人数">
				        	<InputNumber :min="0" v-model="baseInfo.xjzrs"></InputNumber>
				        </FormItem>
				        <FormItem label="影响户数">
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getWfBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { mc, dz, jbr, jbrdh, jbryx, lngAndLat, jzjg, jssj, wfjdjb, jzmj, xjzrs, yxhs, hz, aqzyd } = this.form
					this.baseInfo = { mc, dz, jbr, jbrdh, jbryx, lngAndLat, jzjg, wfjdjb, hz, aqzyd }
					this.baseInfo.jssj = jssj ? new Date(jssj) : ''
					this.baseInfo.jzmj = jzmj ? Number(jzmj): 0
					this.baseInfo.xjzrs = xjzrs ? Number(xjzrs): 0
					this.baseInfo.yxhs = yxhs ? Number(yxhs): 0
			...
			async submit() {
				...
				let { status_code, message } = await api.addWfBase(params);
				if(status_code == 200) {
