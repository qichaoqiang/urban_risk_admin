				...
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.qd"></lng>
				        </FormItem>
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getGczzBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
			...
			async submit() {
				...
				let { status_code, message } = await api.addGczzBase(params);
				if(status_code == 200) {
