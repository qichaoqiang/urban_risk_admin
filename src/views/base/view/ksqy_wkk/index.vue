<template>
	<div class="page">
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="尾矿库地址" prop="wkkdz">
				        	<Input clearable v-model="baseInfo.wkkdz" placeholder="尾矿库地址"></Input>
				        </FormItem>
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng id="lng_box" :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.wkkdz"></lng>
				        </FormItem>
				        <FormItem label="尾矿库范围" prop="wkkfw">
				        	<div @click.stop="openAreaModal">
			        			<Input 
			        				readonly 
			        				v-model="baseInfo.wkkfw" 
			        				icon="md-pin" 
			        				placeholder="尾矿库范围" />
			        		</div>
				        </FormItem>
		...
		<Modal title="企业经纬度标注" v-model="showLngModel" width="1000">
		...
		<Modal title="企业范围标注" v-model="showAreaModel" width="1000">
	...
	export default {
		...
		computed: {
			rules() {
				...
				return {
                	wkkdz: [{ required: true, message: '请输入', trigger: 'change' }],
     ...
     //            	wkkfw: [{ required: true, message: '请选择', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { qymc, tyshxydm, zcdz, sfzgyyq, yqmc, hydm, aqscxkz, zgrs, glrysl, ksjk, yxzt, jbr, jbrdh, jbryx, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, wkkdz, wkkfw, kcsj, jyfw, quyu: [], zdmj }
