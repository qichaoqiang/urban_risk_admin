			...
			<Row type="flex" justify="center" style="height: 100%">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="120" style="width: 600px">
				        ...
				        <FormItem label="竣工投用时间" prop="jgtysj">
	...
	export default {
		...
		computed: {
			rules() {
				...
				const validatorzdLng = (rule, value, callback) => {
					if (!this.baseInfo.zdlngAndLat) {
	                    callback(new Error('请输入'));
	                } else {
	                    callback();
	                }
				...
				return {
					...
					qd: [{ required: true, message: '请输入', trigger: 'change' }],
                	// lngAndLat: [{ required: true, validator: validatorLng, trigger: 'change' }],
                	zd: [{ required: true, message: '请输入', trigger: 'change' }],
                	// zdlngAndLat: [{ required: true, validator: validatorzdLng, trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { mc, jbr, jbrdh, jbryx, fzr, fzrdh, fgaqfzr, fgaqfzrdh, dz, sjdw, sjdwtyshxydm, sgdw, sgdwtyshxydm, gldw, tyshxydm, tjxq, qd, zd, jgtysj: '', lngAndLat: '', zdlngAndLat: '', }
			...
			async submit() {
				let params = {
					...this.baseInfo,
					jgtysj: this.baseInfo.jgtysj ? getDate(new Date(this.baseInfo.jgtysj).getTime(), 'date') : '',
