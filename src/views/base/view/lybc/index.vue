		...
		computed: {
			rules() {
				return {
                	...
                	jydyfw: [{ required: true, message: '请输入', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo = { qymc, hydm, jyfw, tyshxydm, yxzt, zcdz, qyfzr, qyfzrdh, fgaqfzr, fgaqfzrdh, jbr, jbrdh, jbryx, jydz, zgrs, glrysl, clsj, lngAndLat: '' }
			...
			async submit() {
				...
				let { status_code, message } = await api.addLybcBase(params);
				if(status_code == 200) {
					...
					// }else {
					// 	let { status_code, data } = await api.getLybcBase()
					// 	if(status_code == 200) {
