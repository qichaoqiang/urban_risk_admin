<template>
	<div class="page">
			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="建成时间" prop="jcsj">
							<DatePicker type="date" v-model="baseInfo.jcsj"  placeholder="请选择"></DatePicker>
				        </FormItem>
						<FormItem label="物业单位" prop="wydw">
				        	<Input clearable v-model="baseInfo.wydw" placeholder="物业单位"></Input>
				        </FormItem>
				        <FormItem label="地址" prop="dz">
				        	<Input clearable v-model="baseInfo.dz" placeholder="地址"></Input>
				        </FormItem>
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.dz"></lng>
				        </FormItem>
				        <FormItem label="地域范围" prop="dyfw">
					...
					<Form :model="baseInfo" ref="contactInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        <FormItem label="物业负责人">
				        	<Row type="flex" :gutter="20">
					        	<Col span="8">
						        	<FormItem prop="wyfzr">
					        			<Input clearable v-model="baseInfo.wyfzr" :data="areaList" placeholder="姓名"></Input>
					        		</FormItem>
				        		</Col>
				        		<Col span="16">
					        		<FormItem prop="wyfzrdh">
					        			<Input clearable v-model="baseInfo.wyfzrdh" placeholder="电话"></Input>
					        		</FormItem>
				        		</Col>
				        	</Row>
				        </FormItem>
		...
		<Modal width="1000" title="地域范围标注" v-model="showAreaModel">
	...
	export default {
		...
		computed: {
			rules() {
				...
				return {
                	...
                	// wydw: [{ required: true, message: '请输入', trigger: 'change' }],
                	// jcsj: [{ required: true, type: 'date', message: '请输入', trigger: 'change' }],
                	// tyshxydm: [{ required: true, message: '请输入', trigger: 'change' }],
                	// dyfw: [{ required: true, message: '请输入', trigger: 'change' }],
                	...
                	// wyfzr: [{ required: true, message: '请输入', trigger: 'change' }],
                	// wyfzrdh: [{ required: true, message: '请输入', trigger: 'change' }],
		...
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					this.form = data;
					let { xqmc, tyshxydm, hydm, jbr, jbrdh, jbryx, wyfzr, wyfzrdh, fgaqfzr, fgaqfzrdh, lngAndLat, dz, wydw, dyfw, jcsj } = this.form
					this.baseInfo = { xqmc, tyshxydm, hydm, jbr, jbrdh, jbryx, wyfzr, wyfzrdh, fgaqfzr, fgaqfzrdh, lngAndLat, dz, wydw, dyfw, jcsj: '' }
					this.baseInfo.jcsj = jcsj ? new Date(jcsj) : ''
					this.baseInfo.dyfw = dyfw || ''
