			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="涉及区域">
				        	<Input clearable v-model="baseInfo.sjqy" placeholder="涉及区域"></Input>
				        </FormItem>
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng id="lng_box" :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.scdz"></lng>
				        </FormItem>
				        <FormItem label="风险等级数量Ⅰ">
				        	<InputNumber :min="0" v-model="baseInfo.fxdjsl1" placeholder="风险等级数量Ⅰ"></InputNumber>
				        </FormItem>
				        <FormItem label="风险等级数量ⅠI">
				        	<InputNumber :min="0" v-model="baseInfo.fxdjsl2" placeholder="风险等级数量ⅠI"></InputNumber>
				        </FormItem>
				        <FormItem label="风险等级数量III">
				        	<InputNumber :min="0" v-model="baseInfo.fxdjsl3" placeholder="风险等级数量III"></InputNumber>
				        </FormItem>
				        <FormItem label="风险等级数量Ⅳ">
				        	<InputNumber :min="0" v-model="baseInfo.fxdjsl4" placeholder="风险等级数量Ⅳ"></InputNumber>
				        </FormItem>
				        <FormItem label="主体结构建设状态">
				        	<Input clearable v-model="baseInfo.ztjgjczt" placeholder="ztjgjczt"></Input>
				        </FormItem>
				        <FormItem label="主体结构预计建成时间">
				            <DatePicker type="date" v-model="baseInfo.ztjgyjjcsj"  placeholder="主体结构预计建成时间"></DatePicker>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getDtBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { fxymc, dz, sjqy, fxdjsl1, fxdjsl2, fxdjsl3, fxdjsl4, ztjgjczt, ztjgyjjcsj, bz, cfqksm } = this.form
					this.baseInfo = { fxymc, dz, sjqy, ztjgjczt, bz, cfqksm }
					this.baseInfo.fxdjsl1 = fxdjsl1 ? Number(fxdjsl1) : 0
					this.baseInfo.fxdjsl2 = fxdjsl2 ? Number(fxdjsl2) : 0
					this.baseInfo.fxdjsl3 = fxdjsl3 ? Number(fxdjsl3) : 0
					this.baseInfo.fxdjsl4 = fxdjsl4 ? Number(fxdjsl4) : 0
					this.baseInfo.ztjgyjjcsj = ztjgyjjcsj ? new Date(ztjgyjjcsj) : ''
			...
			async submit() {
				let params = {
					...
					ztjgyjjcsj: this.baseInfo.ztjgyjjcsj ? getDate(new Date(this.baseInfo.ztjgyjjcsj).getTime(), 'date') : '',
				...
				let { status_code, message } = await api.addDtBase(params);
				if(status_code == 200) {
