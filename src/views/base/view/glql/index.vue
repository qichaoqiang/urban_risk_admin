				...
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="120" style="width: 600px">
				        ...
				        <FormItem label="经纬度" prop="lngAndLat">
				        	<lng id="lng_box_qd" :lngAndLat.sync="baseInfo.lngAndLat" :dz="baseInfo.dz"></lng>
				        </FormItem>
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getGlqlBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
			...
			async submit() {
				...
				let { status_code, message } = await api.addGlqlBase(params);
				if(status_code == 200) {
