...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getDzzhyhdBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { mc, dz, lngAndLat, jzjg, jzmj, yxrs, yxhs, hz, aqzyd } = this.form
					this.baseInfo = { mc, dz, lngAndLat, jzjg, hz, aqzyd }
					this.baseInfo.jzmj = jzmj ? Number(jzmj): 0
					this.baseInfo.yxrs = yxrs ? Number(yxrs): 0
			...
			async submit() {
				...
				let { status_code, message } = await api.addDzzhyhdBase(params);
				if(status_code == 200) {
