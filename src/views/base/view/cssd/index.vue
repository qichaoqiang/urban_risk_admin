					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="120" style="width: 600px">
				        ...
				        <FormItem label="管理单位">
				        	<Row type="flex" :gutter="20">
				        		...
				        		<Col span="12">
				        			<FormItem prop="gldwtyshxydm">
				        			<Input clearable v-model="baseInfo.gldwtyshxydm" placeholder="统一社会信用代码"></Input>
					        		</FormItem>
				        		</Col>
				        	</Row>
				        </FormItem>
				        ...
				        <FormItem label="隧道简介" prop="sdjj">
				        	<Input clearable type="textarea" :rows="4" v-model="baseInfo.sdjj" placeholder="隧道简介"></Input>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
	...
	export default {
		...
		computed: {
			rules() {
				...
				const validatorJgsj = (rule, value, callback) => {
					if (!this.baseInfo.jgsj) {
						callback(new Error('请选择'));
	                } else {
	                    callback();
	                }
				...
				const validatordzqhdjfzjsdg = (rule, value, callback) => {
					if (!this.baseInfo.dzqhdjfzjsdg && this.baseInfo.dzqhdjfzjsdg !== 0) {
				...
				return {
                	...
                	// sdjj: [{ required: true, message: '请输入', trigger: 'change' }],
                	...
                	// dzqhdjfzjsdg: [{ required: true, type: 'number', validator: validatordzqhdjfzjsdg, trigger: 'change' }],
				}
			},
		},
		methods: {
			async getBaseInfo() {
				...
				if(status_code == 0) {
					...
					this.baseInfo.sgdwtyshxydm = sgdwtyshxydm || ''
					this.baseInfo.sdjj = sdjj || ''
					this.baseInfo.jbr = jbr || ''
					this.baseInfo.jbrdh = jbrdh || ''
					this.baseInfo.jbryx = jbryx || ''
					this.baseInfo.fzr = fzr || ''
					this.baseInfo.fzrdh = fzrdh || ''
