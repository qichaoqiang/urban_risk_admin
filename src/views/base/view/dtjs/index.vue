			...
			<Row type="flex" justify="center">
				<Col>
					...
					<Form :model="baseInfo" ref="baseInfo" :rules="rules" hide-required-mark label-position="left" :label-width="140" style="width: 600px">
				        ...
				        <FormItem label="建设单位" prop="jsdw">
				            <Input clearable v-model="baseInfo.jsdw" placeholder="建设单位"></Input>
				        </FormItem>
				        ...
				        <FormItem label="换乘站数量">
				            <InputNumber :min="0" v-model="baseInfo.hczsl"></InputNumber>
				        </FormItem>
					</Form>
				</Col>	
			</Row>	
...
<script>
	...
	export default {
		...
		methods: {
			async getBaseInfo() {
				let { status_code, data, message } = await api.getDtjsBase(this.gkdx_id);
				if(status_code == 0) {
					this.form = data;
					let { jbr, jbrdh, jbryx, dtxlmc, jsdw, sgdw, sjdw, xlqdz, zdz, quyu_id, lxr, lxrdh, xlc, czsl, hczsl } = this.form
					this.baseInfo = { dtxlmc, jsdw, sgdw, sjdw, xlqdz, zdz, lxr, lxrdh, jbr, jbrdh, jbryx }
					this.baseInfo.xlc = xlc ? Number(xlc) : 0
					this.baseInfo.czsl = czsl ? Number(czsl) : 0
					this.baseInfo.hczsl = hczsl ? Number(hczsl) : 0
					this.getQy()
					this.getHy()
				}
			},
			...
			async submit() {
				...
				let { status_code, message } = await api.addDtjsBase(params);
				if(status_code == 200) {
