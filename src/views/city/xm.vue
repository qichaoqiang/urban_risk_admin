<template>
	<div class="container" style="position: relative;">
		<div style="">
				...
				<Col span="24">
					...
					<Table ref="xmTable" :columns="xmColumns" :data="xmData" :loading="xmLoading">
						...
						<template slot-scope="{ row }" slot="action">
				            <Button type="primary" size="small" ghost style="margin-right: 5px" @click="overviewXm(row)">总览</Button>
			...
			<Modal width="820" :title="`${modelTypeList[modeType - 1]}项目`" :mask-closable="false" v-model="showXmModel" @on-visible-change="xmModelChange">
				<Form :disabled="modeType == 3" :model="xmForm" ref="xmForm" :rules="xmFormRules" hide-required-mark label-position="left" :label-width="120">
			        ...
			        <FormItem label="地图标题" prop="lxdh">
			            <Input clearable type="tel" v-model="xmForm.dtbt"></Input>
			        </FormItem>
			        <FormItem label="项目简介" prop="xmjj">
			            <Input clearable type="textarea" :rows="4" v-model="xmForm.xmjj"></Input>
			        </FormItem>
			        <FormItem label="初始显示范围" prop="csdtfw">
			            <rect-range :qyfw.sync="xmForm.csdtfw"></rect-range>
			        </FormItem>
...
<script>
	...
	import rectRange from './components/rectRange'
	...
	export default {
		name: 'xm',
		components: { partTitle, lng, baseInfo, rectRange },
